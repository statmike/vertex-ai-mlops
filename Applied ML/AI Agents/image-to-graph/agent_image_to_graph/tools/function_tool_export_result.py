...
async def export_result(tool_context: tools.ToolContext) -> str:
    ...
    try:
        ...
        if not nodes:
            return "Error: Graph has no nodes. Analyze the image and build the graph before exporting."
        ...
        return f"Error exporting result: {str(e)}"
...
def _auto_generate_schema(graph: dict, nodes: list[dict], edges: list[dict]) -> dict:
    ...
    for name, types in node_fields.items():
        ...
        if name in required_node:
            ...
            node_model_fields[name] = (Optional[pt], Field(None, description=f"Node {name}"))
    ...
    for name, types in edge_fields.items():
        ...
        if name in required_edge:
            ...
            edge_model_fields[name] = (Optional[pt], Field(None, description=f"Edge {name}"))
