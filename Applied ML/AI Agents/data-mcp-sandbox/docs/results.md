...
Every measured result, with the cells behind it. What each arm *is* — which
server, which tools — is in [paths](paths.md).

Unless a section says otherwise the numbers come from the main capture:
twelve arms × fifteen questions × two governance tiers × five replicates =
**1,800 cells**, `gemini-3.7-flash` at temperature 0, graded on the twelve
questions the oracle can score. `results/report.md` is the generated original;
this page is the reading of it.

The headline — governance roughly triples accuracy on every path — is stated in
the README. This page starts one level below that, with which *part* of
governance bought it.
...
`thinking_mode` — `FAST` or `THINKING` — is the one execution knob the direct CA
arms expose and the MCP arms do not ([paths](paths.md#path-4--managed-agent)).
