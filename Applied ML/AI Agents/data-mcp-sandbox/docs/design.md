...
Both arms were swept on 2026-09-07 and again on 2026-09-13, and are in the
published capture.
...
**25% the cheap way at tier 0 and 16% the dear way at tier 1**, with identical
tier-1 accuracy — so the tool list is worth almost nothing, and what little it is
worth has no consistent sign. Together those turned "managed costs more" into "verbose tool
...
| **Rule acquisition is unmeasurable on Path 4.** | Acquisition asks whether the agent *read* the governed rule, and a Path 4 arm has no tool trace to read it in — so the report prints `--`, never `0%`. Going direct does not fix this and cannot: on `p4_bq_direct_ctx` the glossary is injected by construction, so scoring it as acquired would be reading our own request payload back. Evidence recall *was* in this row and left it — the direct arms disclose a query on 100% of cells against 58–62% for `p4_looker_ca` ([measured](results.md#what-is-opaque-and-what-that-costs-the-measurement)). |
| **The published Path 4 result partly describes the wrapper.** | Probed 2026-09-07 and re-scored 2026-09-13. Same service, same corpus, same model, transport the only difference: **10.5s against 32.6s** median at tier 1, and on the twelve gradeable questions **58/60 against 60/60** — the latency gap is large, the accuracy gap is two cells and favours the wrapper. The 13-point accuracy gap this row reported until 2026-09-13 was entirely the three anchor-ambiguous questions and is withdrawn; re-asking those three with the anchor pinned did not bring it back. Latency is still enough to mean a Path 4 row measured through Toolbox's tool is a property of the tool as much as of Conversational Analytics. This was the finding most worth having and least worth wanting; it is why both arms exist. |
| **A direct arm has no tools and no local model turn, so several metrics are undefined on it.** | Tool count, schema characters, the tool-call sequence the Equivalence check compares, and latency per tool call are all *absent* rather than zero. So are its **token** columns: `usage.py` truthfully records 0 because nothing local ran, and printing that 0 would rank the least visible arm cheapest. Schema size is recorded as 0 so the schema-versus-cost correlation keeps its denominator; the rest print `--` under a `service only` coverage label. |
| **The direct-arm cells come from a later date.** | Swept 2026-09-07 against the 2026-09-05 capture: same corpus, different day and endpoint condition. Within-pair comparisons are unaffected — the transport pair and the context pair each ran inside one sweep — but a direct-arm latency median against a Path 1 one is weaker than a within-sweep comparison, and should be read as such. The oracle moved between the two dates and the merge machinery keeps one per run; had it not, four questions would have graded correct answers wrong and it would have looked like the API being inaccurate. See [`method.md`](method.md#merging-runs-without-flattening-them). |
| **Path 4's cost is partly off-book.** | Conversational Analytics runs its own model loop server-side and does not report it. For the Looker arm Cloud Monitoring recovers it (a 22× understatement). For the BigQuery arm **no meter this project can read reports it at all** — measured, not assumed. See [`paths.md`](results.md#p4_bq_ca-reports-nothing-which-is-not-the-same-as-spending-nothing). |
| **One corpus, one scale — but no longer one model.** | A sibling capture on `gemini-3.8-flash` (10 MCP arms, both tiers, n=3, 720 cells, zero failures) pairs 36 cells per arm/tier against the published run. Kendall tau is **+1.00 at both tiers with zero inversions** above the 8.9-point floor: absolute accuracy moves, the ordering does not. Three deltas resolve above the floor, and two of them are both Path 2 arms at tier 0 (+11.1 each); a matched same-model control re-ran those two arms eight days later and reproduced the published numbers **exactly, +0.0 on every cell**, so that gain is the model and not capture drift. The corpus is still small enough that query cost never forces a strategy change, and that is untested. |
