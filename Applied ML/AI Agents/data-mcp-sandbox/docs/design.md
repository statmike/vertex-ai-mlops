...
| **The published Path 4 result partly describes the wrapper.** | Resolved 2026-09-07, and against us: `p4_bq_direct` does **not** reproduce `p4_bq_ca`. Same service, same corpus, same model, transport the only difference — 88% against 75% accuracy and 10.3s against 32.8s at tier 1. So every Path 4 row measured through Toolbox's tool is a property of the tool as much as of Conversational Analytics, and the direct arms are the ones to cite for the service. This was the finding most worth having and least worth wanting; it is why both arms exist. |
