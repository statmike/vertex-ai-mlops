...
| Question | Answer |
|---|---|
| Does governance help? | **Yes, and it is the largest effect measured.** Accuracy roughly doubles from tier 0 to tier 1 on every path — 33%→75% on `p1_toolbox`, 15%→35% on `p4_looker_ca`. Which *part* of it helps is not measured: tier 1 turns on descriptions, profile scans, rules, a glossary, quality scans and a semantic layer together. `LADDER=1` splits them into five rungs and that sweep has not been run. |
