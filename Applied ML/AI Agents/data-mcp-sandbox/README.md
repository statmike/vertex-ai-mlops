...
| Question | Answer |
|---|---|
| Does governance help? | **Yes, and it is the largest effect measured.** Accuracy roughly triples from tier 0 to tier 1 on every path — 33%→100% on `p1_toolbox`, 12%→43% on `p4_looker_ca`. On the three questions that turn on a **governed definition**, the separation is total: **0 correct out of 180 cells at tier 0**, across every one of the twelve arms, against 138/180 at tier 1. Not one arm answers a governed-definition question correctly without governance. |
| Which *part* of it pays? | **Only one rung is governance, and only one path can use it.** Tier 1 turns six channels on at once; `LADDER=1` splits them into five cumulative rungs over 1,800 cells, zero failures. The twelve questions turn out to be two populations. On the **eight that need no governed definition**, column descriptions alone take every arm from 30–50% to 97.5–100% at rung 1 and the remaining rungs do nothing — that is documentation, and it is the best-value rung on the ladder. On the **questions that turn on a governed definition**, descriptions are worth **0 points** and rung 3 (business rules) takes all three Path 3 arms from **0% to 100%**. Not a curve — a step. Profile scans move nothing on either population, and governance pays for itself: every arm's tokens per correct answer drop 3.6–8.1×. Chasing a failed replication check also found a defect in our own corpus — "trailing 30 days" never pinned its *anchor*, so three questions scored near-arbitrarily depending on where the oracle's window happened to fall. Those three are marked unscoreable and **re-issued as three new questions that pin the anchor**, swept across all twelve arms and merged in, so the main capture grades **twelve of fifteen** questions and the governed-definition population is three questions rather than one. The ladder capture was not re-swept, so the step-function claim above still rests on **one** question at five replicates — which is why it is stated separately from everything else. [Full ladder →](docs/paths.md#which-rung-pays) · [the defect →](docs/paths.md#the-trailing-window-questions-are-anchor-ambiguous) |
| Is it enough? | **No, and for Path 1 writing its own SQL no amount of it is.** From rung 1 onward *every arm that shows its work* acquires the governed rule on 100% of cells — read from the tool results, not from the answer. Yet **`p1_managed` and `p1_matched` answer the governed-definition questions correctly 0 times in 20** at tier 1 in the main capture, and 1 time in 50 at every ladder rung including the top one. The captured SQL says why: it selects Active users from the event table but writes no window at all, so "an event in the trailing 30 days" becomes "appears in the log," which is every user. It is not approximating the governed answer; it is answering a different question. The third Path 1 arm looks like a counterexample and is the opposite of one: **`p1_toolbox` scores 5/5 on both governed-definition questions, and on exactly those two it reached for `ask_data_insights` in every single cell** — Conversational Analytics, borrowed through a Path 1 tool surface. On the one anchored question needing no governed rule it never called it and was right with plain SQL. Acquiring a rule and applying it are different problems, and on this corpus **something that already encodes the rule — a semantic layer, or a service with one behind it — is the only thing that makes a written rule executable.** |
...
| Where does the cost come from? | **Tool schema verbosity, not tool count.** Schema characters predict an arm's median tokens at r = 0.95 (tier 0) and r = 0.99 (tier 1); tool count predicts nothing at r = 0.12 and r = 0.11. One managed `get_table_info` declaration is 78,214 chars — 64% of its arm's prompt floor, and 120× the self-hosted equivalent that does the same job. Splitting input from output shows it directly: `p1_managed` spends **1,870,343 input tokens per correct answer against 5,934 output**. Essentially none of the bill is the model thinking. It is tool definitions, re-sent every turn. |
| How long does it take? | **Latency is a separate axis — it does not track cost.** Tokens against wall clock correlates at r = 0.28 and r = 0.33 — weakly positive, against r = 0.95+ for schema size against tokens, and an arm that costs 100× more does not take 100× longer. Every MCP arm spends 4.3–5.5s per tool call whatever its schema size, so latency is turn count times a constant. Path 4 takes 1–3 calls and pays 20–70s for each, because the loop moved server-side. Measured as **seconds per correct answer**, which is what a user actually waits, tier 1 runs 36–64s for every MCP arm except `p4_looker_ca` at 199s, against 11s for the two direct arms — and at tier 0 the Looker arm needs **24 minutes per right answer**, 4–7× worse than anything else. |
| Does wrapping a service in MCP cost anything? | **Yes — 3.1× the wall clock. Not accuracy.** `p4_bq_ca` and `p4_bq_direct` put the same questions to the same Conversational Analytics API over the same BigQuery corpus; one reaches it as a tool inside an ADK agent, the other calls it. At tier 1 they are two cells apart — the wrapper 60/60, the direct client 58/60 — and the direct client answers in a **10.5s median against 32.6s**, 11s per correct answer against 36s. (The rubric in force before 2026-09-13 put the direct client 13 accuracy points ahead; that gap was entirely the three anchor-ambiguous questions and is withdrawn. The latency is unaffected and stands.) The wrapper also runs a local model loop the direct call does not: 5,941 input tokens per tier-1 cell against none. This is the one pair in the experiment where the data source, the service, and the model are all identical and only the transport differs. |
| Does injecting business context help, and by how much more than noise? | **Nothing the oracle can measure — and the control is what tells us so.** `p4_bq_direct` and `p4_bq_direct_ctx` differ only by a glossary payload, and at tier 0 that payload is empty by design — so those 120 cells send byte-identical requests and are an accidental A/A control. They land **one cell apart** (13/60 vs 14/60). At tier 1, with the glossary injected, they land **one cell apart again** (58/60 vs 57/60). The entire 7-point effect this pair reported earlier lived in the three anchor-ambiguous questions: the glossary moved the agent onto the same window anchor our oracle happens to use, which is a real thing for a glossary to do and is not accuracy. Most published context-injection deltas have no control next to them at all. |
| Going direct — what does it actually buy you? | **Not a choice of model. A choice of how hard it thinks, and that knob is worth ~2.4× on latency.** `ChatRequest.model` is an enum with one selectable value, so neither route lets you pick a model; only the direct route exposes `thinking_mode`. Across three more captures (720 cells), `FAST` runs a **7.4s median against `THINKING`'s 18.4s** — and `FAST` is *flat*, 6.5–7.7s no matter which question you ask, where the default and `THINKING` both scale with difficulty. On a governed warehouse `FAST` costs no measurable accuracy; ungoverned it costs ~16 points. `THINKING` is the one that surprises: it loses ~16 points against `FAST` at tier 1 on the context-injected arm, entirely by applying a governed rule *too* literally. |
...
| Does any of this survive a new model? | **The ordering does, exactly.** Every MCP arm was re-run on `gemini-3.8-flash` — both tiers, 720 cells, zero failures — and paired against the published `gemini-3.7-flash` capture. Kendall tau is **+1.00 at both tiers with zero inversions** above the noise floor: absolute accuracy shifts a few points in both directions, but which architecture beats which does not move. Only three deltas clear the floor, two of them both Path 2 arms at tier 0 (+11.1 each) — so the *same* two arms were re-swept on the *original* model eight days later, and reproduced the published numbers **+0.0 on every cell**. That is the model, not capture drift. At tier 1 eight of ten arms score identically under both models, because governance has already put them on a ceiling. [Full result →](docs/paths.md#does-the-ranking-survive-a-new-model) |
...
accuracy — a gap with no consistent sign. The endpoint is the whole cost story; the tool count
...
service" and "the wrapper we reached it through" stop being one variable. The
answer is that the wrapper costs 3.1× the wall clock and no accuracy: 60/60 for
the wrapper against 58/60 for the direct client at tier 1.
