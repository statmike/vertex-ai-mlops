...
def resolve_goldens(
...
) -> dict[tuple[str, str, int], dict[str, golden.Resolved]]:
    """The oracle for every (arm, question, tier) in the capture, frozen if the file carries it.
    ...
    Keyed by arm and question, not only by tier, because a merged capture holds
    cells from runs on different days, and four of this corpus's oracle values
    are trailing windows that move with the calendar. Arm alone was enough until
    a run could add questions to arms already captured; question is what tells
    those cells apart now (`traces.goldens_by_cell`). Every pair of an unmerged
    capture maps to the same block, so nothing changes for one.
    ...
    """
    present = {(cell.config, cell.question_id) for cell in cells.values()}
    frozen = traces.goldens_by_cell(meta, present)
    if frozen:
        missing = sorted(present - set(frozen))
        if missing:
            raise SystemExit(
                f"This capture freezes an oracle for some cells but not {missing}. "
        ...
        print(
            ...
            f"across {len({config_key for config_key, _ in frozen})} arms)"
        )
        return {
            (config_key, question_id, int(tier)): {
                key: golden.Resolved(**value) for key, value in entries.items()
            }
            for (config_key, question_id), block in frozen.items()
    ...
    return {
        (cell.config, cell.question_id, cell.tier): live[cell.tier] for cell in cells.values()
    }


def score_all(
    cells: dict[str, traces.Cell], resolved: dict[tuple[str, str, int], dict[str, golden.Resolved]]
    ...
    for key, cell in cells.items():
        ...
        scores[key] = scoring.score_cell(
            cell,
            question.evidence,
            resolved[(cell.config, cell.question_id, cell.tier)].get(question.golden_key),
