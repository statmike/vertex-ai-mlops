"""Evaluate Cboe questions with dual-mode comparison (THINKING vs FAST).
...
Run from the data-onboarding project root:
    ...
    uv run python examples/cboe/evaluate.py                      # run all
    uv run python examples/cboe/evaluate.py --persona "Data Analyst"  # one persona
    uv run python examples/cboe/evaluate.py --id data-analyst-q3     # one question
    uv run python examples/cboe/evaluate.py --resume                 # skip completed
    uv run python examples/cboe/evaluate.py --report                 # just build report

Results are saved to examples/cboe/results/eval_results.json.
"""
...
logger = logging.getLogger("cboe_eval")
...
async def evaluate_question(runner, question: dict) -> dict:
    ...
    # Import here to avoid circular issues with early .env loading
...
async def main():
    parser = argparse.ArgumentParser(description="Evaluate Cboe questions with dual-mode comparison")
    ...
    parser.add_argument("--write", action="store_true", help="Write report into cboe.md")
    args = parser.parse_args()

    if args.report:
        cboe_md = Path(__file__).parent / "cboe.md" if args.write else None
        markdown = build_report(RESULTS_FILE, output_path=cboe_md)
        if not args.write:
            print(markdown)
        return
    ...
    if "cboe" not in chat_scope:
        logger.warning(
            ...
            "Set CHAT_SCOPE=data_onboarding_datashop_cboe_com_bronze in .env.",
            chat_scope,
        )
    ...
    logger.info("Run with --report to preview, or --report --write to update cboe.md")
