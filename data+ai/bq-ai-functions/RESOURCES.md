...
Every generative function below documents its supported models as "any GA or preview Gemini model." That is accurate but hides two things worth knowing before you pick one. This section is the single place they are recorded; the per-function entries do not repeat them.
...
| `endpoint` | STRING | Optional | `gemini-2.5-flash` | Any GA or preview Gemini model. BigQuery auto-resolves the full endpoint from model name -- see [Choosing a Gemini Model Endpoint](#choosing-a-gemini-model-endpoint) for the 3.x multi-regional restriction. |
