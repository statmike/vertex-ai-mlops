...
- [ ] `functions/ai_causal_effect/` — **`AI.CAUSAL_EFFECT`** (Preview). CausalImpact-style intervention analysis: it fits an `ARIMA_PLUS` counterfactual on the pre-intervention series and reports the gap afterward. It takes **no control series and no covariates at all**, and the docs give the reason — preventing bias from experiment spillover effects. That is the sharpest possible contrast with the sibling project's `workflows/difference_in_differences/` and `workflows/synthetic_control/`, which are built entirely on control units, and it is the spine of the notebook.
  - **Verified live 2026-09-09 on the same Texas mask-mandate data those two workflows use**: weekly −282.6 absolute / −22.05% relative / p = 0.305; daily −1105.3 / −50.9% / p = 0.107. `output_time_series => TRUE` returns per-row `is_post_intervention`, actual, `predicted_<data_col>`, `lower_bound`, `upper_bound` — everything needed to draw the CausalImpact plot.
  - **The honest finding is a three-method convergence that stays inconclusive**: this agrees in direction with DiD (−19.3/week) and synthetic control (−19.28/week), and **neither granularity reaches p < 0.05**. Publish that as measured — the interesting result is that three structurally different estimators land in the same place and none of them clears significance.
  - **Syntax gotcha:** the intervention timestamp must be a literal. `TIMESTAMP '2020-07-03'` works; `TIMESTAMP("2020-07-03")` is rejected with *"expects the intervention_timestamp argument to be a TIMESTAMP literal, but TIMESTAMP was provided."*
  - **Relationship to `AI.KEY_DRIVERS`**, which the user asked about directly: both are TVF shortcuts that hide a `CREATE MODEL` — `AI.KEY_DRIVERS` wraps contribution analysis plus `ML.GET_INSIGHTS`, `AI.CAUSAL_EFFECT` wraps `ARIMA_PLUS` forecasting. They answer different questions and should not be merged: `AI.KEY_DRIVERS` asks *which segments explain a metric change* (cross-sectional, no time axis required, max 12 dimensions, summable metric only, output segment column named `drivers`), `AI.CAUSAL_EFFECT` asks *whether a known intervention at a known time changed the trajectory*. Worth a short "which one do I want" section in both notebooks.
