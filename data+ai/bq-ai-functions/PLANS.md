...
**Step 1 — Prepare.** Review the audit log to see what was last checked and when. Note any Google Cloud Next announcements or blog posts since the last audit. **Then read *Tracked open questions — the Preview watch list* below** — every row is a question already put to Google or a measurement that documentation could invalidate, and each carries the impact map to follow if the answer has landed.
