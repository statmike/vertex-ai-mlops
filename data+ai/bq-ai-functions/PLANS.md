...
| Vector / search indexes | `VECTOR_INDEX.STATISTICS` (drift), `ALTER VECTOR INDEX REBUILD` (no-downtime rebuild), `PARTITION BY` on TreeAH indexes | **GA 2026-04-29 — documented, not covered here.** Queued for build | [Release notes](https://docs.cloud.google.com/bigquery/docs/release-notes) · [Manage vector indexes](https://docs.cloud.google.com/bigquery/docs/vector-index) | This project creates vector indexes in eight files and manages none of them. `workflows/catalog_search/` runs a real `TREE_AH` hybrid index at 100% coverage — a live index to read real drift statistics from rather than a synthetic one. Add examples to `functions/vector_search/` (notebook + `.sql`), a `RESOURCES.md` section, and a `setup/README.md` note. **Not a new workflow.** |
| Search indexes | `ALTER SEARCH INDEX` DDL for updating index configuration | **Preview 2026-07-13 — not covered here.** Queued with the row above | [Release notes](https://docs.cloud.google.com/bigquery/docs/release-notes) | Same unit of work as vector index lifecycle: build it alongside, in `functions/ai_search/`. |
...
| P6 | **`AI.EVALUATE` defaults to TimesFM 2.0 while `AI.FORECAST` defaults to 2.5.** The two do not match unless set explicitly | Published as a gotcha | The defaults being aligned, or documented | `functions/ai_evaluate/`, `functions/ai_forecast/`, `workflows/time_series_intelligence/`, `RESOURCES.md`, skill `reference/predictive-ai.md` |
