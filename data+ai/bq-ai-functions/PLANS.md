...
repository absent. A *relative* reference that points outside the project does not travel.
...
4. **elsewhere in this repository — only as an absolute public URL**, e.g.
   `https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/...`

What has to travel is the **link**, not the folder. A *relative* path out of the project
breaks the moment the skill is extracted and is the violation; the GitHub URL for the same
file resolves for any reader, inside the repo or out, so it is allowed (refinement made by
the user 2026-09-15). Other parts of the repo may link *in* relatively; this project reaches
back out only through absolute URLs, and prefers a native equivalent or public documentation
where one exists.

Two cases are treated differently:

- A **link** (`[text](target)`) is followable, so a *relative* outward one always violates.
  Remedy it by retargeting, not by deleting — to the native equivalent, to public
  documentation, or to the absolute GitHub URL for the same file.
- A **mention** (a backticked path in prose) violates only when the target still exists
  outside the project — that is a live pointer a reader will chase, with nothing to click.
  A mention of a path that no longer exists is *retirement provenance*: it records where
  content came from and there is nothing to follow, so it stays.
...
- **`PLANS.md`** is forward-looking, repo-bound, and never shipped in a skill. It may
  cite anything, including sources outside the repository. It is exempt from the
  *target* policy only — its `#anchor` links are still checked, because a citation
  pointing at a heading that no longer exists is broken wherever it lives.
- **`README.md`** may link to `agent-skills/`, the artifact built from this project.

Enforced by `agent-skills/tooling`. Three checks, all cheap enough to run on every change:

```bash
cd agent-skills

# 1. Outward links, dead links, and dead #anchors.
PYTHONPATH=tooling/src python3 -m agent_skills_tooling.cli check-links \
  ../data+ai/bq-ai-functions --sibling ../data+ai/bq-ml --repo-root ..

# 2. RESOURCES.md (the index) and reference/ agree: no orphan page, no dead index
#    entry, one H1 per page, filename matching that H1's slug.
PYTHONPATH=tooling/src python3 -m agent_skills_tooling.cli check-reference \
  ../data+ai/bq-ai-functions

# 3. No narrative/*.md drifts from the notebook it was generated out of.
PYTHONPATH=tooling/src python3 -m agent_skills_tooling.cli check-narratives \
  .agents/skills --repo-root ..
```

The anchor half of check 1 earns its keep now that the reference is split: a link to
`reference/<page>.md#heading` still resolves to the *file* after that heading is renamed,
so nothing but the anchor check notices the reader landed at the top of the page.

Check 3 is driven by `source_project` in each `skill.manifest.json`; a skill without that
field, or without a `narrative/`, is skipped.

Both projects currently pass all three with zero errors.
...
1. Run all three checks above — `check-links`, `check-reference`, `check-narratives` —
   each must report zero errors
2. Regenerate any affected `agent-skills/` narrative in the same commit
   (`check-narratives` is what catches it when this is forgotten)
...
### Phase 11: AI.CAUSAL_EFFECT and the Cloud AI service models (planned 2026-09-10, in progress)

> **Cadence, set by the user 2026-09-11:** this phase and [`bq-ml`'s Phase 9](../bq-ml/PLANS.md#phase-9-coverage-completion--a-home-inside-this-project-for-every-ml-function-planned-2026-09-10-in-progress) are one build round, so they share one queue — build straight through, pre-execute and commit each folder as it finishes, and queue it for **reading** in [*Pending Reviews*](../bq-ml/PLANS.md#pending-reviews) rather than starting a second list. No Restart & Run All is needed to populate outputs; the committed notebook already carries them. **This project needs the second pre-execution more than `bq-ml` does** — every `AI.*` function is non-deterministic, `AI.EVALUATE` and `AI.DETECT_ANOMALIES` measurably so even with the model version pinned, so any published number must be one that survived two runs. **Superseded by the user 2026-09-11** for the four Cloud AI service model folders — *"lets keep buildign nteoobks … and then run all at the end of building"*. The template-error risk it guarded against is handled instead by each notebook executing cleanly end to end before it is committed, and by the family reference page being written from the four notebooks' measured outputs rather than from the template.

Found in the same 2026-09-10 audit that produced [`bq-ml`'s Phase 9](../bq-ml/PLANS.md) — the two phases are one batch of work split across the projects by function prefix, and they cross-reference each other. **The four Cloud AI service model folders, the family reference page and the setup work are built as of 2026-09-11; the *Enhanced existing content* and *Cross-project* rows are not.** Every notebook change carries its matching markdown updates (`reference/*.md`, `RESOURCES.md`, `README.md`, this file, and the `agent-skills` mirror) in the same commit.

**New function**

- [x] **`AI.CAUSAL_EFFECT`** (Preview) — **built 2026-09-11, and it landed in [`bq-ml/workflows/causal_effect/`](../bq-ml/workflows/causal_effect/), not here.** This project keeps outbound links only. See *[Placement exception](#placement-exception-aicausal_effect-is-documented-in-bq-ml)* below for the decision and its carriers.
  - **The plan's headline finding was wrong, and the build corrected it.** The row this replaces claimed the three estimators "land in the same place." They do not. Measured on one panel, in identical units: `AI.CAUSAL_EFFECT` **−56.52/week**, DiD **−19.29/week**, synthetic control **−19.28/week** — same sign, **~2.9×** the magnitude. The −282.6 cumulative figure was right; dividing it by the 5 post-intervention weeks was the step never taken, and "agrees in direction" was allowed to stand in for "agrees." **Lesson worth keeping: a cumulative effect and a per-period effect are not comparable quantities, and "same direction" is not convergence.**
  - The real finding is better than the one planned: a univariate counterfactual cannot level off when a common shock crests, so it charges the entire nationwide summer-2020 surge to the mandate. The disagreement *is* the lesson about identifying assumptions. Still inconclusive on significance — p = 0.3048 clears nothing, as planned.
  - **What the build established that the plan did not know** (all verified live 2026-09-11, all now in [`bq-ml/reference/model-free-functions.md#aicausal_effect`](../bq-ml/reference/model-free-functions.md#aicausal_effect)): the counterfactual is **bit-for-bit** `ARIMA_PLUS` + `ML.FORECAST` on defaults (5 forecasts and 10 bounds all exactly equal); both effect columns reproduce by hand as plain sums; the p-value reproduces under **none** of seven constructions (closest 0.09% off in standard-deviation terms, and the residual changes sign across post-window lengths); the p-value is deterministic across cache-disabled runs and invariant to `confidence_level`; there is no `model` argument, so the engine cannot be swapped for TimesFM; no model artifact is created (model counts and `INFORMATION_SCHEMA.JOBS` both checked).
  - **Syntax gotcha (confirmed):** the intervention timestamp must be a literal. `TIMESTAMP '2020-07-03'` works; `TIMESTAMP("2020-07-03")` is rejected with *"expects the intervention_timestamp argument to be a TIMESTAMP literal, but TIMESTAMP was provided."*
  - **Relationship to `AI.KEY_DRIVERS`**, which the user asked about directly: both are TVF shortcuts that hide a `CREATE MODEL` — `AI.KEY_DRIVERS` wraps contribution analysis plus `ML.GET_INSIGHTS`, `AI.CAUSAL_EFFECT` wraps `ARIMA_PLUS` forecasting. They answer different questions and should not be merged: `AI.KEY_DRIVERS` asks *which segments explain a metric change* (cross-sectional, no time axis required, max 12 dimensions, summable metric only, output segment column named `drivers`), `AI.CAUSAL_EFFECT` asks *whether a known intervention at a known time changed the trajectory*. Shipped as a comparison table in [`reference/augmented-analytics.md`](reference/augmented-analytics.md), since Google's shared "Augmented analytics" heading actively invites the confusion.

#### Placement exception: `AI.CAUSAL_EFFECT` is documented in `bq-ml`

**Decided by the user 2026-09-11**, choosing "bq-ml only, AI list links out" over building it in both places. Recording it here so it does not later read as an oversight in the `AI.*` coverage.

The project split is *"does the reader manage a model artifact,"* not *"does the name start with `AI.`"* Two things put this function on the other side of that line:

1. Its subject is causal inference, and the only way to judge its estimate is against the five causal estimators already in `bq-ml/workflows/`. The notebook re-derives DiD and synthetic control on the same panel to do exactly that — impossible to do well from here.
2. It is the one `AI.*` function that is **not** foundation-model backed. Its counterfactual is measurably plain `ARIMA_PLUS`, it rejects `model`, and it is deterministic — none of which is true of anything else in this project.

**Completeness is preserved by links, not by duplication.** The carriers, all updated in the same commit:

| Carrier | What it holds |
|---|---|
| [`README.md`](README.md) Augmented Analytics table | A row for `AI.CAUSAL_EFFECT` marked "covered in the sibling project," with notebook/SQL/reference links and a blockquote giving the reason |
| [`README.md`](README.md) "How Functions Relate" diagram | Listed in the AUGMENTED ANALYTICS box with a `→ covered in bq-ml` marker |
| [`RESOURCES.md`](RESOURCES.md) Sections list | The Augmented Analytics bullet names it and links out |
| [`reference/augmented-analytics.md`](reference/augmented-analytics.md) | A full pointer section plus the `AI.KEY_DRIVERS` vs `AI.CAUSAL_EFFECT` comparison table |
| `agent-skills` `bigquery-ai-functions` reference | Sibling-skill routing to `bigquery-ml`; no narrative here, since there is no notebook here |

**The rule this sets for future audits:** an `AI.*` function may be documented in `bq-ml` when it manages no artifact *and* its subject is already owned there. Any such case must appear in this project's function catalog with an outbound link, so the catalog stays a complete inventory of the `AI.*` surface. This is the only instance so far.

**New function category — Cloud AI service models (the remote API-based family)**

Four `ML.*` functions that call a pre-trained Google API through a remote model. They came out of the [`bq-ml` Phase 9 audit](../bq-ml/PLANS.md) as having no home anywhere, and they land here rather than there — **user-confirmed 2026-09-10**, with the reasoning recorded in both files.

**Why here, and why as their own group:** each requires `CREATE MODEL … REMOTE WITH CONNECTION … OPTIONS(REMOTE_SERVICE_TYPE = …)` over a Cloud AI service, documented on its own page — [*The CREATE MODEL statement for remote models over Cloud AI services*](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service). The full `REMOTE_SERVICE_TYPE` enum is `CLOUD_AI_VISION_V1`, `CLOUD_AI_SPEECH_TO_TEXT_V2`, `CLOUD_AI_TRANSLATE_V3`, `CLOUD_AI_NATURAL_LANGUAGE_V1`, `CLOUD_AI_DOCUMENT_V1` — and **that fifth value is `ML.PROCESS_DOCUMENT`, which this project already covers in [`functions/ml_process_document/`](functions/ml_process_document/), already contrasted against `AI.PARSE_DOCUMENT`.** So the family already has a member here, already built in the old-API-versus-LLM shape these four need. That, plus this project's existing habit of hosting service-calling `ML.*` functions (`ml_generate_text/`, `ml_generate_embedding/`, `ml_process_document/`), is why the split is by nature rather than by prefix. `bq-ml/models/remote/` keeps only a pointer section; folding these into it would blur the custom-Vertex-endpoint branch with this one.

**The through-line for all four notebooks:** these are *remote API-based models* — the model object is a thin handle on a pre-trained Google API. Nothing trains, there are no weights, and there is no `ML.EVALUATE`. Each notebook shows the legacy call, then the same job done with an `AI.*` function on an LLM, and says plainly which to reach for in new work and why the old one still exists (deterministic per-field output, a fixed taxonomy, per-call pricing, and no prompt to maintain are real reasons to keep it).

- [x] **`functions/ml_annotate_image/`** — built 2026-09-11. `CLOUD_AI_VISION_V1`, over an object table. Vision features: `FACE_DETECTION`, `LANDMARK_DETECTION`, `LOGO_DETECTION`, `LABEL_DETECTION`, `TEXT_DETECTION`, `DOCUMENT_TEXT_DETECTION`, `IMAGE_PROPERTIES`, `OBJECT_LOCALIZATION`; returns `ml_annotate_image_result` (JSON) plus a per-row `ml_annotate_image_status`. **Contrast:** `AI.GENERATE_TABLE` / `AI.CLASSIFY` over the same `ObjectRef` images, which return typed columns instead of a JSON blob to unpack. A documented known issue to carry over honestly: rows can come back with a non-empty status even on a successful job.
  - **What the build established that the plan did not know:** the documented eight-feature list is what is *supported*, not what the argument *parses*. Measured — `SAFE_SEARCH_DETECTION` and `CROP_HINTS` are accepted and return `safe_search_annotation` / `crop_hints_annotation`; `WEB_DETECTION` is accepted and returns **no keys at all**; an invented name is rejected with *"Found unsupported value … of setting field `vision_features`"*. Accepted is not supported, and the silent-empty case is the one that will look like a data problem.
  - **A zero-valued JSON field is omitted from the response entirely.** Landmark bounding boxes are absolute pixels, and a vertex at the origin arrives as `{}` — `INT64(vertex.x)` returns `NULL`, not `0`.
- [x] **`functions/ml_transcribe/`** — built 2026-09-11. `CLOUD_AI_SPEECH_TO_TEXT_V2`. **Contrast:** `AI.GENERATE` over audio, where Gemini both transcribes and answers questions about the audio in one call.
  - **The plan had the argument shape wrong.** `SPEECH_RECOGNIZER` is an option on the *model*, and the per-call configuration is a **named argument on the function** — `recognition_config => JSON '…'`, not a `STRUCT`. It is the only family member that breaks the `STRUCT` convention, and it is **required** unless the model carries a recognizer. Both mistakes print the signature: `ML.TRANSCRIBE(MODEL, TABLE, [recognition_config => JSON])`.
  - **`chirp` is the only accepted `model`** — `chirp_2`, `long`, `short`, `telephony` all rejected with *"Supported models are: chirp."* And a `SPEECH_RECOGNIZER` is accepted only from `asia-southeast1`, `us-central1` or `europe-west4`.
  - **`language_codes` is echoed, not enforced** (forcing `en-US` on French audio returns French, still reporting `en-US`); some `features` keys void the row with *"Config contains unsupported fields"* naming no field; and **the function is not reproducible** — four cache-off calls on the same clip returned two distinct transcripts.
  - **No reservation is involved.** An earlier premise that this family needed one was falsified before building: `INFORMATION_SCHEMA.JOBS_BY_PROJECT` reports no reservation and edition `none` for every call. That constraint belongs to `bq-ml`'s image preprocessing functions, not here.
- [x] **`functions/ml_translate/`** — built 2026-09-11. `CLOUD_AI_TRANSLATE_V3`. Note the real input-shape trap: the function reads a column literally named `text_content` and errors if it is absent, so a query form with an alias is the practical calling convention. **Contrast:** `AI.GENERATE` with a translation prompt — the tradeoff is genuine, with the Translation API giving stable, cheap, per-language behaviour and the LLM giving tone and context control.
  - **The IAM finding the setup page does not carry:** the documented pair (`serviceusage.serviceUsageConsumer`, `bigquery.connectionUser`) is not enough. With both granted the call still fails — *"Cloud IAM permission 'cloudtranslate.languageDetectionModels.predict' denied"* — until `roles/cloudtranslate.user` is added. Measured by withholding each role in turn.
  - **`target_language_code` is a scalar, not a column.** A column reference fails to compile with `Unrecognized name`; fanning out over languages is one call per language.
- [x] **`functions/ml_understand_text/`** — built 2026-09-11. `CLOUD_AI_NATURAL_LANGUAGE_V1`; entity, sentiment, and syntax analysis. **Contrast:** `AI.CLASSIFY` (fixed taxonomy), `AI.SCORE` (sentiment as a number), and `AI.GENERATE_TABLE` (entity extraction into typed columns) — this is the family member the `AI.*` surface most completely supersedes, and the notebook says so.
  - **The per-row failure is real and measured here:** `ANALYZE_ENTITY_SENTIMENT` supports fewer languages than `ANALYZE_SENTIMENT`, so Korean and Vietnamese rows return `INVALID_ARGUMENT: The language ko is not supported for entity_sentiment analysis.` **while the job succeeds** — 2 of 8 rows in the notebook. This is the family's retry pattern, shown live rather than described.
  - **Two roles is the whole list**, because Cloud Natural Language has no predefined IAM role of its own — the counter-example that makes the family's IAM matrix worth publishing.
- [x] **Grouping work, done 2026-09-11:** a new [`reference/cloud-ai-service-models.md`](reference/cloud-ai-service-models.md) covering the shared `CREATE MODEL`/connection/IAM setup once, a new category in `README.md` and `RESOURCES.md` alongside Generation / Managed / Embeddings & Search / Predictive AI / Document Processing, and a cross-reference from `functions/ml_process_document/` naming it as the fifth member. **Settled 2026-09-10: `ml_process_document/` stays in Document Processing** — it is genuinely a document tool, it is wired into two workflows, and re-categorising it would be churn for no reader benefit. It is named as the fifth family member by cross-reference from the new category, and the four new folders follow the pattern it already set.
  - Carriers, all in the same commit: the new reference page; a **Cloud AI Service Models** category in [`README.md`](README.md) with all five rows and a box in the *How Functions Relate* diagram; the four folders in *Project Structure*; a bullet in [`RESOURCES.md`](RESOURCES.md)'s Sections list; a family cross-reference table in `functions/ml_process_document/`; and pointer sections in the sibling project at [`bq-ml/models/remote/`](../bq-ml/models/remote/) (notebook + `.sql`), [`bq-ml/reference/create-model-model-types.md`](../bq-ml/reference/create-model-model-types.md)'s `REMOTE` entry, and its README row.
- [x] **`setup/README.md`** — done 2026-09-11. Four rows added to *Which Functions Need What*; the `REMOTE_SERVICE_TYPE` form added to *Remote Models* (with the note that `CREATE MODEL` succeeding is not evidence the connection can call anything); object tables extended to `ML.ANNOTATE_IMAGE` and `ML.TRANSCRIBE`; and *For ML.PROCESS_DOCUMENT* under Permissions replaced by **For the Cloud AI service models**, carrying the per-service API and role matrix for all five.
  - **A gap the docs work surfaced and fixed in code:** `ml_translate`, `ml_understand_text` and `ml_process_document` ran their `AI.*` comparison examples without ever granting `roles/aiplatform.user` — they worked only because the shared project's connection service account already held it from the other notebooks. In a fresh project those cells would fail. The role is now in each notebook's grant list (and each teardown), flagged as belonging to the comparison rather than to the function.
- [x] **`agent-skills` mirror** — done 2026-09-11, in the same commit. `bigquery-ai-functions` gained `reference/cloud-ai-service-models.md` (an eighth bucket) and four narratives; `ml_process_document.md` was regenerated for the notebook's new sibling table and corrected grant loop; `SKILL.md` gained a seventh routing branch, a cross-cutting gotcha, the reference-file line, and a function count moved 25 → 29. Skill at 0.4.0. **Reciprocity ran in both directions rather than one:** the sibling `bigquery-ml` skill's `SKILL.md` and `reference/model-management.md` now open the `REMOTE WITH CONNECTION` fork explicitly (`endpoint =` is a Vertex AI Endpoint you pay for by the hour, consumed with `ML.PREDICT`; `REMOTE_SERVICE_TYPE =` is these five), and `narrative/remote.md` was regenerated for the notebook cell that says the same thing — 0.3.6. **And the triage skill needed a new branch, not a new example:** `choosing-a-bigquery-ai-approach` frames the whole decision as trained-vs-generative, which left "pre-trained Cloud AI service" unreachable — it is neither. A tenth question and a head-to-head bullet fix that; 0.2.0. Suite green: 0 link violations (91 and 129 files), 0 reference errors (8 and 5 pages), 104/104 narratives current, `validate --all` OK on all three skills.

**Enhanced existing content**

- [x] `workflows/metric_diagnostics/` — an `AI.CAUSAL_EFFECT` step: once `AI.KEY_DRIVERS` has said *where* a metric moved, quantify whether a known intervention caused it. **Done 2026-09-15**, as a new Step 5 with four cells — and it ships the *opposite* of what the row asked for, on purpose. **There is no known intervention in the Citi Bike data**, so running the function on it and reporting an effect would have published exactly the error the step is supposed to teach against. It is a **placebo test** instead: a 426-day daily-hours series, six intervention dates chosen only for spacing, `num_post_intervention_points => 60` so the six are comparable. **One of the six clears `p < 0.05` — 2017-10-01 at 0.044108 — with nothing behind it.** A counterfactual fitted on a spring-and-summer ramp cannot know ridership falls every autumn, so it carries the summer level forward and reads the seasonal turn as an intervention. The general statement, which is the reusable part: a univariate counterfactual can only carry forward patterns already in its pre-window, so an annual cycle needs more than a year of pre-intervention history before it is one of them. **A data-shape constraint found the hard way and worth recording:** `citibike_trips` has **no rows between Oct 2016 and Mar 2017**. A window of 2017-01-01..2017-04-30 therefore has zero pre-intervention points, and `AI.CAUSAL_EFFECT` reports that as *"Time series data col is empty"* — not as a row count problem. Three type variations (INT64/FLOAT64/TABLE-vs-subquery) were ruled out before the gap was found; the continuous stretches are 2013-07→2016-09 and 2017-04→2018-05. Also in this change: the header's numbered list gained item 5 and the narrate step renumbered 5 → 6 in five places, the Functions-used line gained `AI.CAUSAL_EFFECT`, and `workflow_metricdiag_daily` joined the cleanup loop. **Notebook re-executed end to end**; the placebo table above is live output.
- [x] `functions/ai_key_drivers/` — the "which one do I want" cross-link described above. **Done 2026-09-15**, one bullet in the notebook's *Alternatives* list: both are TVF shortcuts hiding a `CREATE MODEL`, but one wraps contribution analysis and answers *which segments*, the other wraps `ARIMA_PLUS` and answers *whether an intervention changed the trajectory* — plus the trap the shared "Augmented analytics" heading invites, that running key drivers on before-versus-after periods is not a causal estimate. Points at the sibling notebook and at `reference/augmented-analytics.md` for the full table. Markdown-only, no re-run.
- [x] `functions/ai_forecast/` — note that `AI.CAUSAL_EFFECT` is the same ARIMA_PLUS engine pointed at a counterfactual instead of the future. **Done 2026-09-15**, one bullet in *Alternatives*, framed as "the same forecasting idea aimed backwards": it is **not** TimesFM, it wraps `ARIMA_PLUS` (the sibling notebook reproduces its counterfactual bit-for-bit with a plain `CREATE MODEL`), and it forecasts past a point *inside* the data rather than past the end of it. That distinction is the reason it sits next to `AI.FORECAST` in the docs and nowhere near it in mechanism. Markdown-only, no re-run.
- [x] `functions/ai_evaluate/` — side-by-side with the sibling project's planned `bq-ml/functions/evaluation/` (`ML.METRICS`). Measured 2026-09-10 on one saved forecast: identical MAE (`1218.7306780133927`) and MSE (`2169021.1595780854`), and **those two are the only metrics the sets share** — `AI.EVALUATE` uniquely gives MAPE/sMAPE/MASE/RMSE, `ML.METRICS` uniquely gives r²/MSLE/median AE/explained variance. The structural difference matters more than the metric lists: `AI.EVALUATE` regenerates the forecast internally on every call, `ML.METRICS` scores a saved artifact. **Done 2026-09-15**, one bullet in the notebook's *Alternatives* list, leading with the structural difference and then the metric-set split. **The two 16-digit measurements above stayed out of the notebook on purpose.** They were taken in a probe, not by a cell, and no cell in `ai_evaluate.ipynb` renders them — which is exactly the prose-only-number defect that a Restart & Run All cannot catch. Making them live would mean adding an `ML.METRICS` cell and re-executing, and `AI.EVALUATE` is measured **not reproducible run to run even with `model` and `context_window` pinned** (Step 3 of that same notebook), so the identity is not a property a re-run can be relied on to reproduce. The bullet therefore says MAE and MSE are the only shared names and that they were measured identical once, with a pointer to the reproducibility caveat below it. The digits stay here, in the planning record, where their provenance is stated. Markdown-only, no re-run.

**Cross-project — tracked in both files**

- [x] `bq-ml/workflows/difference_in_differences/` and `bq-ml/workflows/synthetic_control/` — Related Content bridge and a short "a third estimate" note. **Done 2026-09-11 in `bq-ml`** (markdown-only, one cell each, no re-run); this copy of the row went unchecked until 2026-09-15 — the hazard of a row tracked in two files. Verified 2026-09-10 that no honest-finding prose in either needs retracting: their claims are about staggered-timing group-time estimators and constrained quadratic optimization, neither of which `AI.CAUSAL_EFFECT` provides.

**Documentation URLs** for everything in this phase are in the *Documentation URLs* table under *Maintenance & Audit* below.
...
### Phase 13: The `AI.CAUSAL_EFFECT` p-value mechanism — closing C1 and C2 (planned 2026-09-16)

**Trigger.** An answer to the Preview watch-list questions arrived from outside this repo and supplied the p-value formula: `p = 2·(1 − CDF(|Z|))`, `Z` = cumulative causal impact ÷ sum of pointwise standard error, explicitly described as assuming perfect positive correlation across forecast horizons and therefore conservative. It also stated the CausalImpact lineage (BSTS replaced by `ARIMA_PLUS`), the direct-inference implementation with no model artifact, the spillover rationale for having no covariates, and a three-point minimum shared with `ARIMA_PLUS` / `ML.TREND` / `ML.SEASONALITY`.

**What we did with it, before planning any edit.** Took the formula at face value and re-derived offline against rendered outputs. It reproduces exactly once the CDF is evaluated with an approximation indistinguishable from **A&S 26.2.18** — see the resolved **C1** row above for the full numbers. That turns an answer into an identification, and it turns our published Step 7 conclusion into a retraction.

#### 13a. What must change, and why (the retraction)

| Claim we published | Status | Replacement |
|---|---|---|
| The reported p-value sits **outside** the range any correlation structure among the model's forecast errors can produce | **Wrong** | It sits exactly where the stated formula puts it. The perfect-correlation sum *is* the variance, by design |
| "The cumulative-effect variance is not a linear combination of the model's own pointwise forecast errors. Something else is in it — parameter-estimation uncertainty would be the ordinary candidate" | **Wrong, and it is the sentence that matters** | Nothing else is in it. The residual was the tail-area evaluation |
| The horizon sign-straddle rules out a constant correction, a fixed variance formula, and a fixed-df *t* | **Sound premise, inverted use** | The oscillation is the approximation's own error curve — evidence *for* the mechanism, not elimination of alternatives |
| `ML.FORECAST`'s multiplier "fits an approximation to the inverse normal CDF" | **Right, and now closed** | It *is* that approximation, inverted, to `1.2e-6` at 0.95 and across all five levels |
| The `ARIMA_PLUS` equivalence might be an implementation detail | **Refined** | The method is the design; the implementation is direct inference with no model artifact |

#### 13b. The build

- [ ] **`bq-ml/workflows/causal_effect/causal_effect.ipynb`** — the substantive work. Keep all seven constructions in Step 7; the elimination is what makes the answer legible, and the notebook's value is the derivation, not the verdict. Add **one new code cell** that (i) computes `Z = absolute_effect / sum(se)` directly at each of the three horizons — **not** from the `sd implied by p` column, which round-trips through the exact normal inverse and would make the test partly circular — (ii) evaluates `2·(1 − AS(|Z|))` and tables it against the reported values, and (iii) inverts the same approximation to predict Step 6's multiplier at all five confidence levels. Rewrite the Step 7 finding markdown around it. Rewrite Step 6's multiplier finding to point forward instead of ending on "not a Student *t*, not a constant scale factor." Land the threshold argument the way **13c** specifies — as an interpretation lesson, not a warning bullet. Rebalance **Step 8** into the two-sided trade from **C4** — keep the chart, the −56.52 / −19.29 / −19.28 comparison and the identifying-assumption paragraph exactly as they are, and add the spillover half. Name the **CausalImpact-with-`ARIMA_PLUS`-instead-of-BSTS** substitution in the opening and in Step 5. Add the **three-point minimum** to Step 10's limits. **Present tense, no editorial history** — the notebook states the mechanism; this file records that it changed.
- [ ] **Re-execute it** (new cells are real computation), then reconcile every markdown number against rendered output.
- [ ] **`causal_effect.sql`** — the comment blocks at ~231 and ~255–265 carry the superseded conclusion in `--` prose. A backtick-scoped grep will miss them; grep the numbers (`0.304355`, `266.696`, `1.9564581`).
- [ ] **`bq-ml/reference/model-free-functions.md`** — the *`p_value`* row ("**Does not reproduce.**") and the *Interval multiplier* row in the `AI.CAUSAL_EFFECT` evidence table.
- [ ] **`bq-ai-functions/reference/augmented-analytics.md`** — check whether it restates the residual; if it only points at `bq-ml`, leave it.
- [ ] **`bq-ai-functions/workflows/metric_diagnostics/`** — **finding survives, verified**: the placebo `p = 0.044108` is `Z = 2.009605`, exact-normal `0.044473`, still under 0.05. One sentence only, noting the reported p-values are the conservative ones. Do not import the mechanism discussion.
- [ ] **`bq-ml/PLANS.md` line ~430** (Phase 9 entry) — **found by the 2026-09-16 carrier sweep, not in the first draft of this plan.** It restates the wrong conclusion in PLANS' own voice: *"seven constructions fail to reproduce it ... The notebook stops there and claims no mechanism, which is the point."* Rewrite; the "what does this function buy me" answer around it still holds.
- [ ] **Agent skill `bigquery-ai-functions`** — **also found by the sweep.** `reference/driver-analysis.md` line 61 is hand-written (not generated, so no regeneration will fix it) and says *"the `p_value` is not reconstructable and does not move with `confidence_level`."* The second half stays true; the first half is now false.
- [ ] **Agent skill `bigquery-ml`** — `narrative/causal_effect.md` (regenerated), `reference/workflows-and-pipelines.md`, and `SKILL.md` if it carries the open question. **`reference/unsupervised-and-specialized.md` is named in the `bq-ml` multiplier impact map but the sweep found no carrier in it** — check it rather than editing it, and if it is genuinely clean, correct the impact map instead. Then `check-narratives`, `manifest`, `validate`, and the link/reference checkers across both projects.
- [ ] **`README.md` / `RESOURCES.md`** in both projects — grep for the residual before assuming they are clean.

**Carrier sweep, run 2026-09-16 before any edit** (`1.9564581`, `0.304355`, `0.304812`, `266.695866`, `275.139`, "does not reproduce", "not reconstructable", `1.959964`, "prediction interval", `standard_error`):

- **Exactly two notebooks are affected** — `causal_effect.ipynb` substantively, `metric_diagnostics.ipynb` by one sentence. Every residual number lives only in `causal_effect.ipynb`.
- **Five other notebooks consume `ML.FORECAST`/`AI.FORECAST` prediction intervals and are all clean:** `models/arima_plus/`, `models/arima_plus_xreg/`, `functions/ai_forecast/`, `workflows/time_series_intelligence/`, `overview.ipynb`. None states interval arithmetic and — the specific latent defect worth looking for — **none back-derives a standard error from rendered bounds**, which would have been silently 0.18% low. Confirmed by reading the matching lines, not by the absence of a grep hit.
- `difference_in_differences/` and `synthetic_control/` carry only the −19.29 / −19.28 cross-links and need nothing. `ai_forecast/`'s bullet says the counterfactual reproduces bit-for-bit, which remains true.

#### 13c. The threshold argument — how to land it (user's framing, 2026-09-16)

This is the classic significance-threshold conversation, and the notebook should say so rather than treating the approximation as a hazard. The sequence:

1. **Name it.** A p-value read against `0.05` is a **decision boundary** — it collapses a continuous measure of evidence into a binary, and everything except which side you landed on is discarded.
2. **Show why that matters here, from our own measurement.** One approximation, two readings of the same error:

   | Reading | Magnitude of the error | Consequence |
   |---|---|---|
   | As an interval, `confidence_level => 0.95` | half-width `538.298` against a textbook `539.263` — **off by 0.965**, 0.18% | invisible on a chart, immaterial to any interpretation |
   | As a p-value against `0.05` | true `0.05` is reported as `0.049591` | can flip a published conclusion |

   The error is not bigger in the second row. The **threshold** is what makes it consequential, by converting a smooth quantity into a hard one. That is the argument for intervals, and it falls out of a measurement rather than a stylistic preference.
3. **Point at what the function already hands you.** `AI.CAUSAL_EFFECT` returns `lower_bound` and `upper_bound` next to `absolute_effect`. On the Texas panel the 95% interval is roughly `[-821, +256]` — it contains zero, which is the same information `p = 0.30` carries, plus the effect size, plus the direction, plus how much of each is consistent with the data. The p-value is the **least** informative column in the row.
4. **Be honest that the interval inherits the same conservatism.** Both are built from the same `sum(se)`, so the interval is deliberately wide in exactly the way the p-value is deliberately large. The difference is that a wide interval *communicates* that conservatism, while a conservative p-value silently makes you less likely to cross a line you cannot see moving.
5. **Do not moralize.** One paragraph, present tense, no lecture on NHST. The reader gets the tool and the reason.

**A cell to add while doing this, and it is a genuine test rather than a restatement.** If the bounds use the same two ingredients as the p-value, then `absolute_effect ± AS⁻¹((1+cl)/2) · sum(se)` should reproduce `lower_bound`/`upper_bound`. Predicted from our rendered numbers: `[-635.413, +70.200]` at `0.80`, `[-820.904, +255.692]` at `0.95`, `[-994.582, +429.369]` at `0.99`. **Unverified — query it live during the build.** If it holds, the notebook reproduces *every* column of the function's output row from two quantities, which is a much better closing position than reproducing two of three. If it does not hold, that is a new finding and the interval discussion above still stands on its own.

#### 13d. Two things that do **not** go in the repo

1. **The GA feature proposal for covariates.** It is unreleased roadmap and the repo is public. C4 states current Preview behavior and the trade it implies; it must not hint that covariates are coming.
2. **Names, teams, and the internal link** that came with the answer. Standing policy — product state only.

The formula and the CausalImpact lineage are safe to publish: both are statements about how the shipped function behaves today, and every number we print for them is one we measured ourselves.

#### 13e. What stays open after this

C3 (no `model` argument) and C5 (Preview status) are unchanged. C4 stays open as a *capability* question even though its framing improves — there is still no diagnostic. The multiplier row in `bq-ml/PLANS.md` stays on the watch list because the **documentation** gap is untouched: we can now explain the arithmetic, and Google still does not document it.
...
| AI.CAUSAL_EFFECT (Preview — Phase 11) | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-causal-effect |
| CREATE MODEL over Cloud AI services (`REMOTE_SERVICE_TYPE`) — Phase 11 | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service |
| ML.ANNOTATE_IMAGE (Phase 11) | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-annotate-image |
| ML.TRANSCRIBE (Phase 11) | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-transcribe |
| ML.TRANSLATE (Phase 11) | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-translate |
| ML.UNDERSTAND_TEXT (Phase 11) | https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-understand-text |
...
**Preview surface this depends on:** `AI.PREDICT` (Preview), `AI.EVALUATE`'s tabular branch (Preview), `VECTOR_SEARCH`'s single-search/hybrid syntax (Preview, on a GA function), `AI.SEARCH`'s `mode` argument (Preview, on a GA function), `AI.CAUSAL_EFFECT` (Preview, content in `bq-ml`). Two impact maps follow the tables — use them instead of hand-patching one file.
...
#### Augmented analytics — `AI.CAUSAL_EFFECT` (as of 2026-09-11)

**Where the content lives:** [`bq-ml/workflows/causal_effect/`](../bq-ml/workflows/causal_effect/) and [`bq-ml/reference/model-free-functions.md#aicausal_effect`](../bq-ml/reference/model-free-functions.md#aicausal_effect) — see *[Placement exception](#placement-exception-aicausal_effect-is-documented-in-bq-ml)*. The rows are tracked **here** because this is the project that watches the `AI.*` surface for documentation changes; the fixes they trigger land over there.

| # | Open question | Our published position today | What would settle it | What we update when it does |
|---|---|---|---|---|
| ~~C1~~ **RESOLVED 2026-09-16** | **How is `p_value` computed?** — **answered, and the answer retracts our published position.** `p = 2·(1 − CDF(\|Z\|))` where `Z` = cumulative effect ÷ **sum of the pointwise standard errors**, and the CDF is evaluated with an approximation numerically indistinguishable from **Abramowitz & Stegun 26.2.18** (the Hastings quartic, `\|ε\| < 2.5e-4`) rather than the exact normal CDF. Verified against rendered outputs at three post-window horizons: `0.511078994`/`0.735735000`/`0.304812168` against reported `0.511079`/`0.735735`/`0.30481216822794477` — the last agreeing to **bit-exactly** (`0.00e+00` at two of three post-window horizons, one unit in the last place at the third), computed from `282.606186 ÷ 275.139104` with both numbers taken directly from cells rather than back-derived. The *same* approximation, inverted, reproduces `ML.FORECAST`'s `1.9564581` interval multiplier to `1.2e-6` and its full sign-changing sweep — so C1 and the `bq-ml` multiplier row were always one mechanism. **What we published and now retract:** that the implied variance sits *outside* anything a correlation structure among the model's own forecast errors can reach, and that "something else is in it — parameter-estimation uncertainty would be the ordinary candidate." There is nothing else in it. The sum-of-standard-errors choice is the deliberate conservative upper bound (perfect positive correlation across horizons); the 0.09% residual was entirely the CDF approximation, which we had listed as candidate mechanism #2 and ranked second. **Why we got it wrong:** the elimination argument assumed an exact CDF, so a numerical artifact was read as a structural one — and the sign-straddle across three horizons, cited as ruling out whole families, was the approximation's own oscillation, i.e. evidence *for* the answer. **Consequence to publish:** the deliberate conservatism moves p by ~0.0155 on this data; the approximation by ~0.00046, 34x smaller — but it lands on the threshold people use. A true `p = 0.05` is reported as **0.049591**; a reported `0.05` is a true **0.050411**. **Standing constraint on how this is worded:** *numerically indistinguishable from* A&S 26.2.18 at all eight measured points — never a claim about BigQuery's source. | Resolved. Rebuilt as the notebook's Step 7 conclusion 2026-09-16 (Phase 13) | Nothing further — but **watch for the approximation being replaced by an exact CDF**, which would move the 0.95 multiplier to `1.9599640` and shift every p-value in the fourth decimal at once | If it changes: `bq-ml/workflows/causal_effect/` Steps 6–7 (the identification cell is the whole finding), `bq-ml/reference/model-free-functions.md` (*`p_value`* and *Interval multiplier* rows), the `bq-ml` PLANS multiplier row, and the regenerated `narrative/causal_effect.md` |
| ~~C2~~ **ANSWERED 2026-09-16** | **Is the `ARIMA_PLUS` equivalence a guarantee or an implementation detail?** — the method is the documented design, the *implementation* is direct inference that produces no model artifact. So Step 5's bit-for-bit equality (5 forecasts and 10 interval bounds all exactly equal) is an equivalence between two code paths, not an identity, which is a stronger result than we framed it as and a more fragile one to depend on. This also explains the function's lineage cleanly, and it is the single best teaching addition available here: **`AI.CAUSAL_EFFECT` is CausalImpact (Brodersen et al., 2015) with `ARIMA_PLUS` substituted for BSTS.** The notebook already calls it "the CausalImpact idea"; it can now name the substitution and what the substitution costs — BSTS admits covariates and can build a synthetic control, which is exactly the common-shock defense C4 is about | Published as **measured 2026-09-11**; the engine statement upgraded to design-level 2026-09-16 | An engine change, or the equality breaking | Highest-churn row here: Step 5 is *built* on the equality. Re-run the notebook first, then the reference table, then the README row and the `bq-ml` PLANS Phase 9 entry |
| C3 | **No `model` argument.** `model => 'TimesFM 2.0'` fails with *"Named argument model not found in signature for call to function AI.CAUSAL_EFFECT."* So the engine cannot be swapped, and no `ARIMA_PLUS` option (`holiday_region`, `data_frequency`, manual order) is reachable either | Published as a limitation, with "build the model yourself" as the workaround | The argument appearing, or any option passthrough | The reference entry's limitations list and the notebook's Step 10 gotchas |
| C4 | **Nothing warns when a common shock is present.** The function has no control series and no diagnostic that flags one. Measured cost on this project's one panel: **−56.52/week** against DiD **−19.29** and synthetic control **−19.28**, ~2.9x. **Reframed 2026-09-16: this is a trade, not only a weakness.** Univariate forecasting cannot absorb a covariate, so a nationwide shock lands in the estimate — *and* it cannot be contaminated by an intervention that spills into the control units, which is the failure mode of every control-based method when the treatment bleeds across the boundary (geo-targeted marketing across adjacent metros is the customer-facing example). Our Step 8 currently presents only the first half. The reference page already carries the spillover rationale; the notebook does not | Published as the notebook's honest finding, with the level-off-vs-straight-line chart as the evidence and "reach for a control-unit method when one exists" as the guidance — **to be rebalanced into a two-sided trade in Phase 13**, keeping the chart and every number | A covariate/control argument, or a documented diagnostic | Step 8 and its chart, the reference limitation, and the Related-content bridges in `difference_in_differences` and `synthetic_control` |
| C5 | **Preview status.** Arguments and output columns can change; `prob_causal_effect` in particular is pure redundancy (`1 - p_value`) and is the kind of column that gets dropped | All eight arguments and both output shapes published as verified 2026-09-11 | Preview → GA | Re-verify the full signature against the notebook's Step 9 table before swapping the status label — that table is the only place the whole signature is written down |
...
| 2026-09-04 | `RESOURCES.md` split into `reference/`, with `RESOURCES.md` kept in place as the index — plus two new checks that keep the new shape honest | Same change as the sibling `../bq-ml/`, applied here in its own commit; that project's audit log carries the full reasoning and the verification method, and this row records what is specific to this project. `RESOURCES.md` went from **2,683 lines to a 98-line index**, with each top-level section moved to its own page under `reference/`: `general-purpose-functions.md` (603), `embedding-generation-and-semantic-search.md` (669), `unstructured-data-infrastructure.md` (484), `predictive-ai.md` (408), `managed-functions.md` (315), `document-processing.md` (261), `augmented-analytics.md` (122). **`Choosing a Gemini Model Endpoint` was deliberately kept inline** rather than split out — it is orientation a reader needs before picking a section, and two inbound anchors point at it. The index keeps every inbound plain-path reference resolving; only anchored links needed retargeting. Every section body was verified byte-identical to its previous content after heading demotion and link re-basing. **One pre-existing defect found here**: `RESOURCES.md` linked `#ml_process_document`, but GitHub's slug for that heading is `mlprocess_document` — the link had never resolved. **One defect introduced by the retarget and caught before commit**: `README.md:189` became `[RESOURCES.md](reference/predictive-ai.md)`, a link whose text named a file other than its destination; the text is now `[Predictive AI]`. Guardrails added in the same pass and wired into the checklist above — `check-reference`, `check-narratives`, and anchor resolution inside `check-links`, each proved to fail on its own failure mode before being trusted. Post-state: `check-links` 82 files / 0 violations, `check-reference` 7 pages / 0 errors, `check-narratives` 39/39 current, manifest 0.2.4 → 0.3.0, with 16 narratives regenerated. Documentation and tooling only; **nothing re-executed** — all notebook edits are markdown cells, verified cell-for-cell identical to `HEAD` on `(cell_type, execution_count, outputs)`. **Anchor slugs verified against GitHub's renderer on 2026-09-04** — all 14 index and reference pages match exactly (132 anchors, 0 differences); the one defect the test found (GitHub's `-1`/`-2` suffixing of repeated headings, which `anchors_of` collapsed) is recorded in `../../agent-skills/PLANS.md`. |
| 2026-09-11 | `AI.EVALUATE`'s TabFM classification metrics — the `BOOL`/`STRING` convention corrected in six places, from a measurement made in the sibling project | The sibling project built [`bq-ml/functions/evaluation/`](../bq-ml/functions/evaluation/) for `ML.METRICS` and found that **the label column's TYPE silently decides what `precision`/`recall`/`f1_score` mean**: a `BOOL` pair is scored binary — the positive (`TRUE`) class alone — while the `STRING` rendering of the identical values is scored multiclass and macro-averaged, even at two classes; `accuracy` is unaffected. **Rather than assume `AI.EVALUATE` behaved the same way, it was tested here** — four isolated cache-off runs of each on one imbalanced problem (12 positives of 62 rows, TabFM confusion matrix TP 12 / FP 11 / FN 0 / TN 39): the `BOOL` label returned precision `0.5217391304347826` and recall `1.0`, the `STRING` label `0.7608695652173914` and `0.89`, exactly the two-class means. **It follows the identical rule**, which makes this a BigQuery-wide evaluation convention rather than a quirk of either function — the transferable form of the lesson, and the reason it was worth measuring rather than citing. **This project was wrong in six places**, all of which said "macro-averaged" unconditionally: `functions/ai_evaluate/ai_evaluate.sql` and `ai_evaluate.ipynb` (markdown cell 22), `workflows/tabular_prediction/tabular_prediction.ipynb` (markdown cell 16, which **additionally** listed `accuracy` among the macro-averaged metrics — `accuracy` is identical under either type), `reference/predictive-ai.md`'s TabFM classification output table, and the skill's hand-written `reference/predictive-ai.md` (which crossed the 100-line reference budget in the process and now carries a table of contents). All six now split `STRING` (macro) from `BOOL` (binary) with the measured numbers and a pointer to the side-by-side. A seventh nuance published at the same time: **macro F1 is the mean of the per-class F1s, not the F1 of the macro precision and recall** — recomputing it the second way does not reproduce the number (`0.7729` versus `0.7996` on the `ML.METRICS` side). **One self-inflicted error worth recording**, because it is the classic version of the mistake this function invites: the first probe table carried both `is_chinstrap` and `is_chinstrap_str`, and `SELECT * EXCEPT(splits)` handed `AI.EVALUATE` the unused encoding of the label as a perfect-predictor feature. Caught before results returned; the run was killed and redone selecting exactly one label column per call. That leak is now a published gotcha in the sibling notebook. Both notebook edits here are markdown-only — verified as 1-line and 3-line diffs with outputs untouched, so **nothing was re-executed** — and `narrative/ai_evaluate.md` and `narrative/tabular_prediction.md` were regenerated rather than hand-edited (2- and 3-line diffs, no churn). Manifest 0.3.0 → 0.3.1; `validate` OK on all three skills, `check-narratives` 39/39 current, `check-links` 0 violations across 82 files. |
