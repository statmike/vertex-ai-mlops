...
- [ ] `workflows/metric_diagnostics/` — an `AI.CAUSAL_EFFECT` step: once `AI.KEY_DRIVERS` has said *where* a metric moved, quantify whether a known intervention caused it.
- [ ] `functions/ai_key_drivers/` — the "which one do I want" cross-link described above.
- [ ] `functions/ai_forecast/` — note that `AI.CAUSAL_EFFECT` is the same ARIMA_PLUS engine pointed at a counterfactual instead of the future.
- [ ] `functions/ai_evaluate/` — side-by-side with the sibling project's planned `bq-ml/functions/evaluation/` (`ML.METRICS`). Measured 2026-09-10 on one saved forecast: identical MAE (`1218.7306780133927`) and MSE (`2169021.1595780854`), and **those two are the only metrics the sets share** — `AI.EVALUATE` uniquely gives MAPE/sMAPE/MASE/RMSE, `ML.METRICS` uniquely gives r²/MSLE/median AE/explained variance. The structural difference matters more than the metric lists: `AI.EVALUATE` regenerates the forecast internally on every call, `ML.METRICS` scores a saved artifact.

**Cross-project — tracked in both files**

- [ ] `bq-ml/workflows/difference_in_differences/` and `bq-ml/workflows/synthetic_control/` — Related Content bridge and a short "a third estimate" note. Verified 2026-09-10 that no honest-finding prose in either needs retracting: their claims are about staggered-timing group-time estimators and constrained quadratic optimization, neither of which `AI.CAUSAL_EFFECT` provides.
...
| # | Open question | Our published position today | What would settle it | What we update when it does |
|---|---|---|---|---|
| C1 | **How is `p_value` computed?** Six constructions were tested against the reported `0.30481216822794477` and none reproduces it. Closest is the sum of the pointwise standard errors — perfectly correlated forecast errors — at `0.303489`, 0.27% off. Two constraints on any answer, both measured: the p-value is **invariant to `confidence_level`** (0.80/0.95/0.99 identical while interval widths move), so it comes from the model's internal variance rather than the rendered bounds; and it is **deterministic** across cache-disabled repeat calls, so it is a closed form, not a posterior sample the way R's `CausalImpact` computes its tail area | Published as *does not reproduce*, with the six-construction table shown and **no mechanism claimed** — explicitly including "not a vendor bug." The notebook states that the significance test is therefore the one part of the function not reconstructible from documented BigQuery pieces | A documented formula, or a released detail of the variance model | Step 7 of `causal_effect.ipynb`, the reference entry's measured-behavior table, and the README row. If a formula lands and one of the six constructions was right, the notebook's framing changes from "not reconstructible" to "reconstructible, here is how" — a rewrite of the step, not a patch |
...
| C3 | **No `model` argument.** `model => 'TimesFM 2.0'` fails with *"Named argument model not found in signature for call to function AI.CAUSAL_EFFECT."* So the engine cannot be swapped, and no `ARIMA_PLUS` option (`holiday_region`, `data_frequency`, manual order) is reachable either | Published as a limitation, with "build the model yourself" as the workaround | The argument appearing, or any option passthrough | The reference entry's limitations list and the notebook's Step 9 gotchas |
