...
| # | Open question | Our published position today | What would settle it | What we update when it does |
|---|---|---|---|---|
| C1 | **How is `p_value` computed?** Seven constructions were tested against the reported `0.30481216822794477` and none reproduces it. Closest is the sum of the pointwise standard errors — perfectly correlated forecast errors — at `0.304355`, 0.09% off in standard-deviation terms. The *statistically correct* construction, the ψ-weight cumulation of an `ARIMA(0,2,0)`'s five forecast errors, is further away at `0.289301`, so the implied variance sits **outside** the range any correlation structure among the model's own forecast errors can reach. Swept over three post-window lengths the residual **changes sign** (0.999418 / 0.999073 / 1.000946 against that ceiling), which rules out a fixed multiplier, a fixed variance formula, and a t with fixed df in one measurement. **That conclusion depends on one input being right:** the standard errors must come from `ML.FORECAST`'s `standard_error` column, not from the rendered interval bounds, because those bounds use a multiplier of `1.9564581` rather than the normal quantile — tracked as its own open question in [`bq-ml/PLANS.md`](../bq-ml/PLANS.md#tracked-upcoming) with an impact map. Reconstructed errors put all three horizons on the same side and manufacture a consistent sign. Two constraints on any answer, both measured: the p-value is **invariant to `confidence_level`** (0.80/0.95/0.99 identical while interval widths move), so it comes from the model's internal variance rather than the rendered bounds; and it is **deterministic** across cache-disabled repeat calls, so it is a closed form, not a posterior sample the way R's `CausalImpact` computes its tail area | Published as *does not reproduce*, with the seven-construction table shown and **no mechanism claimed** — explicitly including "not a vendor bug." The notebook states that the significance test is therefore the one part of the function not reconstructible from documented BigQuery pieces | A documented formula, or a released detail of the variance model | Step 7 of `causal_effect.ipynb`, the reference entry's measured-behavior table, and the README row. If a formula lands and one of the seven constructions was right, the notebook's framing changes from "not reconstructible" to "reconstructible, here is how" — a rewrite of the step, not a patch |
| C2 | **Is the `ARIMA_PLUS` equivalence a guarantee or an implementation detail?** The counterfactual is bit-for-bit `ARIMA_PLUS` + `ML.FORECAST` on defaults today — 5 forecasts and 10 interval bounds all exactly equal. Nothing documents that, and a Preview function could switch engines (to TimesFM, say) without a release note | Published as **measured, dated 2026-09-11**, and the notebook's whole Step 5 rests on it | Documentation of the engine, or the equality breaking | This is the highest-churn row here: Step 5 of the notebook is *built* on the equality, so if it breaks the step stops being a proof and becomes a comparison. Re-run the notebook first, then the reference table, then the README row and the `bq-ml` PLANS Phase 9 entry |
...
| C4 | **Nothing warns when a common shock is present.** The function has no control series and no diagnostic that flags one. Measured cost on this project's one panel: **−56.52/week** against DiD **−19.29** and synthetic control **−19.28**, ~2.9× | Published as the notebook's honest finding, with the level-off-vs-straight-line chart as the evidence and "reach for a control-unit method when one exists" as the guidance | A covariate/control argument, or a documented diagnostic | Step 8 and its chart, the reference limitation, and the Related-content bridges in `difference_in_differences` and `synthetic_control` |
