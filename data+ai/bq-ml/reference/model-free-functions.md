...
**Repo example (tested):** `data+ai/bq-ml/functions/evaluation/evaluation.ipynb` and `evaluation.sql` — trains a `LINEAR_REG` on `penguins`, saves predictions, **drops the model**, and shows `ML.EVALUATE` failing with *Not found: Model* while `ML.METRICS` returns the same six metrics; scores a constant baseline the model can be compared against; measures the BOOL/STRING split on one table and reproduces both rows by hand from the confusion matrix; tests the same split in `AI.EVALUATE`; and reproduces `80038528` with the ruled-out mechanisms above. Two workflows in this project already score saved predictions the long way and now cross-link it: `workflows/hierarchical_forecasting/` Step 4 (TEST actuals joined to two reconciled forecasts) and `workflows/regression_based_forecasting/` (its custom metrics helper, used wherever `ML.EVALUATE` does not apply). Both keep their hand-written arithmetic, which is the useful part of the comparison — they report RMSE, MAPE and pMAE, and `ML.METRICS` returns none of the three.
