...
- Normalize numeric features (TRANSFORM + `ML.STANDARD_SCALER`); DNN training is sensitive to feature scale.
- For imbalanced classification use `auto_class_weights=TRUE` (or `class_weights`); the repo fraud example trains with it FALSE and still achieves strong ROC AUC.
- Training is expensive: the repo run trained ~937s over ~16.7 GB processed for `[64,32]` / 10 iterations — start small and use `early_stop`.
...
**BigFrames API:** `bigframes.ml.ensemble`/`bigframes.ml` — `bigframes.ml.ensemble` is trees; for DNN use `bigframes.ml.linear_model`-adjacent? Use `bigframes.ml.compose`/SQL fallback. Closest direct equivalent: `bigframes.ml` does not expose a first-class DNN class in all versions — fall back to `CREATE MODEL` SQL via BigFrames `bpd.read_gbq`/`Session`. (Verify against installed BigFrames version.)
