...
A **Cloud resource connection** is needed for **remote models** — `CREATE MODEL ... REMOTE WITH CONNECTION ...` calling a Vertex AI endpoint (see `models/remote/`) — and for **object tables** over Cloud Storage (see `functions/image/`). In both cases BigQuery acts through the connection's service account rather than yours.

**Correction (verified live, 2026-07-22 — `models/imported/` and `models/export/`):** imported models (`TENSORFLOW`/`TENSORFLOW_LITE`/`ONNX`/`XGBOOST`) and `EXPORT MODEL` do **NOT** need a connection, contrary to what this section previously said. Both read/write GCS using the credentials of whoever runs the `CREATE MODEL`/`EXPORT MODEL` statement — ordinary GCS IAM (`roles/storage.objectViewer` to import, `roles/storage.objectAdmin` to export) on your own account, not a Cloud Resource connection.

**Object tables are the separate case (verified live, 2026-09-11 — `functions/image/`).** An `EXTERNAL TABLE ... WITH CONNECTION ... OPTIONS(object_metadata = 'SIMPLE')` over image files **does** require a Cloud Resource connection, and that connection's service account needs `roles/storage.objectViewer` on the bucket — your own access to the bucket is irrelevant. Two things to expect: the grant takes **minutes** to propagate (a 60-second wait was not enough; retry the first query on a permission error rather than sleeping longer), and the *image functions themselves* require a **reservation** independently of any of this — see [`reference/model-free-functions.md`](../reference/model-free-functions.md#image-preprocessing-functions-mldecode_image-mlresize_image-mlconvert_image_type-mlconvert_color_space). Reservation and connection are orthogonal requirements; earlier wording here conflated them.
