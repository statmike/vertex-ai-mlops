...
| [Random Forest Regressor](models/random_forest_regressor/) | `RANDOM_FOREST_REGRESSOR` | ML.PREDICT | GA | Bagged tree ensemble; same data as Linear/Boosted Tree Regressor — `r2_score` ≈ 0.92 vs. boosting's ≈ 0.98 and linear's ≈ 0.88, and measurably sensitive to `xgboost_version` (≈ 0.74 at the `0.9` default) |
...
| [Imported](models/imported/) | `TENSORFLOW` / `TENSORFLOW_LITE` / `ONNX` / `XGBOOST` | ML.PREDICT | GA | Bring a model trained outside BigQuery (scikit-learn, XGBoost, Keras) in from GCS and run `ML.PREDICT` natively — no connection, no serving endpoint; verified BQML's `XGBOOST` importer accepts a Booster by file extension (`.json`/`.ubj` load, `.bst` is rejected), not by the library version that wrote it |
...
|----------|----------|--------|--------------|
| [Exploration](functions/exploration/) | `ML.DESCRIBE_DATA` / `ML.CORRELATION` | GA / Preview | Profile a dataset and find what moves with the target, before any model exists. `ML.DESCRIBE_DATA` catches what a null count cannot: on `census_adult_income`, `workclass` reports `num_nulls = 0` while 1,836 rows hold the string `' ?'`. `ML.CORRELATION` carries three measured surprises — its `SPEARMAN` is `CORR()` over SQL `RANK()` (competition/min ranks, **not** the mid-ranks SciPy and R use, disagreeing in the second decimal place on tied data), while its `KENDALL` in the same function **is** the tie-corrected tau-b; `dimension_cols` is exactly `GROUP BY CUBE`, matched row-for-row; and `segment_size` is the segment's row count, not the pairwise *n* behind the coefficient next to it. The payoff is the whole point of dimension slicing: the whole-table education/hours correlation of 0.1481 is nobody's correlation — across the 15 segments with 500+ rows it runs 0.0608 to 0.2721 |
...
| [Causal Effect](workflows/causal_effect/) | `AI.CAUSAL_EFFECT` (Preview), `ARIMA_PLUS` · `ML.FORECAST`, `ML.ARIMA_EVALUATE`, `ML.EXPLAIN_FORECAST` | When there is no control group at all, the only counterfactual left is the series' own past — the CausalImpact idea, and BigQuery's one-call version of it. Runs `AI.CAUSAL_EFFECT` on the *same* Texas mask-mandate panel `difference_in_differences` and `synthetic_control` use, then takes it apart: the counterfactual is **bit-for-bit identical** to a plain `ARIMA_PLUS` + `ML.FORECAST` on default options (all 5 forecasts and 10 interval bounds equal under exact float comparison), and both effect columns are reproduced by hand as plain sums — leaving the p-value as the only genuinely new quantity, which **six separate constructions fail to reproduce** (closest misses by 0.27%). Measured alongside: the p-value is deterministic and invariant to `confidence_level`, and the fitted order is `(0,2,0)` with no drift, which is why the counterfactual is a perfect straight line. Honest finding: all three estimators agree on sign and **disagree on magnitude by ~2.9x** (-56.52/week here vs. -19.29 and -19.28) — a univariate counterfactual cannot level off when a nationwide surge crests, so it charges the whole common shock to the mandate. The function's own p-value (0.30) declines to call any of it significant. |
...
| [Managed Airflow (Cloud Composer)](pipelines/composer_airflow/) | Managed Airflow Gen 3 · `BigQueryInsertJobOperator`, `BranchPythonOperator`, XCom | The same `sql_scripting` drift-check/retrain logic on a real, live Managed Airflow Gen 3 environment — `BigQueryInsertJobOperator` for every BigQuery job, `BranchPythonOperator` + XCom for the conditional retrain, a join task with `trigger_rule=NONE_FAILED_MIN_ONE_SUCCESS`. First use of Gen 3 in this repo (cheaper than Gen 2 via explicit per-component `workloads_config` sizing, billed in DCU-hours). **Verified live that `useQueryCache: False` (a real Python bool) works correctly here**, unlike the KFP-components bug found in `vertex_kfp` — confirmed via `INFORMATION_SCHEMA.JOBS_BY_PROJECT`. Also found and worked around a minimal-webserver restart cycle (~every 10-15 min) requiring genuine retry-with-backoff, not just a fixed short retry budget. |
| [Airflow + Vertex AI Pipelines (KFP)](pipelines/airflow_with_kfp/) | `RunPipelineJobOperator` · shares the Managed Airflow Gen 3 environment above | The "meta-orchestration" pairing: one Airflow task (`RunPipelineJobOperator`) triggers `pipelines/vertex_kfp/`'s already-built Vertex Pipeline — the repo's original "DAG 3" pattern, now pointed at a real BQML pipeline instead of Dataflow/Dataproc. Verified live end-to-end, including independently confirming the triggered `PipelineJob` itself succeeded (not just the Airflow task). **A real bug caught building this**: this notebook must recreate its own feature table rather than assume `composer_airflow`'s copy still exists, since that notebook's own Cleanup drops it as soon as its run finishes, even in the same session. Also found that deleting a Managed Airflow environment does **not** delete its GCS bucket — orphaned and billed separately until removed by hand. Performs the real shared-environment teardown. |


> **Naming:** Cloud Composer was renamed **Managed Service for Apache Airflow** (short form *Managed Airflow*) on 2026-04-24, and its generations from "Composer 3/2/1" to **Gen 3 / Gen 2 / Legacy Gen 1**. The API, IAM roles, `gcloud` surface, image-version strings and documentation URLs are unchanged and still read `composer` — so does the `pipelines/composer_airflow/` folder name, which is kept stable for inbound links.
---
...
ML.ROC_CURVE                                   ML.FEATURE_INFO / ML.TRAINING_INFO
   ...
   (SQL scripting · scheduled queries · Managed Airflow · Vertex KFP)
```
...
└── pipelines/              ◄ Orchestration / MLOps (scheduled queries, Managed Airflow, Vertex KFP)
```
