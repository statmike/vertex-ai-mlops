...
- **Two foundation models power the zero-training predictions, and both have moving defaults** — TimesFM for time series (its default version silently flipped 2.0 → 2.5, changing every unpinned forecast's numbers) and TabFM for tabular rows. Pin the version when reproducibility matters, and check the BigFrames wrapper's default separately — it lags the SQL surface. **TabFM has no version to pin and its output is not deterministic**: an identical `AI.PREDICT` call returns slightly different predictions each run, so materialize results instead of re-running them.
