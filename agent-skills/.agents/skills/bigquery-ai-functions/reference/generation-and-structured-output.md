...
- `AI.COUNT_TOKENS` is **text-only, verified in Preview (2026-07-17, re-verified 2026-09-15)** — the one multimodal holdout after the sweep that unwrapped the other eleven functions, so the re-test covered both field-name spellings rather than assuming the rename carried the old result forward: passing a STRUCT prompt with an `object_refs` field (the same pattern that works fine in `AI.GENERATE`), the older `ObjectRefRuntime` spelling of it, or a bare `ObjectRef` all fail with `"Unable to coerce type ... to expected type STRING."` (now nested inside a `No matching signature` error that prints the full signature — which is also the cleanest confirmation of the undocumented `title`/`model` parameters). `full_response.promptTokensDetails[].modality` is therefore always `"TEXT"`. Re-test at GA — `AI.PARSE_DOCUMENT` is precedent for multimodal support arriving without an obvious signature change.
