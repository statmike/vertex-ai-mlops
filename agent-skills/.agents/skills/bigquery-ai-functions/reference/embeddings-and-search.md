...
- Object tables used with remote models for multimodal embedding generation require a **BigQuery reservation**; the inline `OBJ.MAKE_REF(uri, connection)` pattern avoids that requirement entirely and is the preferred approach for one-off multimodal calls. **`OBJ.GET_ACCESS_URL` is not part of it** — the `ObjectRef` goes straight into the `content` argument (verified 2026-09-15); `OBJ.FETCH_METADATA` is only needed if you want the content type or size.
...
- **A search index is a different object with a far larger gate — 10 GiB.** `CREATE SEARCH INDEX` (which powers `SEARCH()`, not `VECTOR_SEARCH`) *succeeds* on a small table and then never populates: `index_status` `TEMPORARILY DISABLED`, `coverage_percentage` 0, `disable_time` equal to `creation_time`, and `disable_reason` naming the threshold `10737418240` bytes. Nothing at demonstration scale clears it, so for keyword matching reach for `lexical_search_columns` on a vector index instead. Note also that `SEARCH_INDEXES.ddl` is normalized rather than a verbatim echo — a statement written with `OPTIONS (analyzer = 'LOG_ANALYZER')` is recorded as `OPTIONS (data_types = ['STRING'])`, with the analyzer in its own column.

## Managing an index — reading state is free, altering is not

Creating an index and managing one need different things from the project. Creating, populating and querying all work on plain on-demand billing; **altering does not**.

- **Three independent health signals, and confusing them leads to the wrong action.** `coverage_percentage` / `unindexed_row_count` (from `INFORMATION_SCHEMA.VECTOR_INDEXES`) say how much of the table is covered; `tfdv_drift` (from `VECTOR_INDEX.STATISTICS`) says how far the *vector distribution* has moved; `last_model_build_time` says when the TreeAH model was last built and is **not** `last_refresh_time`. Measured: inserting 11,780 rows whose vectors duplicate already-indexed ones (7,275 → 19,055) dropped coverage 100 → 38 and set `unindexed_row_count` to 11,780, while drift moved only `0.0` → `1.97e-5` and `last_model_build_time` did not move at all. A flood of new-but-distributionally-identical rows is a **coverage** problem — it resolves on the automatic refresh and is not a reason to rebuild.
- **`VECTOR_INDEX.STATISTICS` takes exactly one argument and it must be a relation.** `VECTOR_INDEX.STATISTICS(TABLE t)`. Adding the index name as a second argument fails with `Signature accepts at most 1 argument, found 2 arguments`; passing a string fails with `argument 1 must be a relation (i.e. table subquery)`. It returns one column, `tfdv_drift` — `null` before the first refresh, `"0.0"` at fresh 100% coverage, and **zero rows** (not an error, 0 bytes scanned) on a table with no index, so it is safe to call unconditionally. A **negative** value is a sentinel, not a magnitude: a freshly built partitioned index reported `-1.0` where an unpartitioned hybrid index reported `"0.0"`. Those two differ in more than one way, so the cause is not isolated — read a negative as *no figure available yet* and corroborate with `coverage_percentage`.
- **Every `ALTER` path requires an Enterprise-edition reservation with a `BACKGROUND` job-type assignment.** `ALTER VECTOR INDEX ... REBUILD`, `ALTER SEARCH INDEX ... SET OPTIONS (...)` and `ALTER SEARCH INDEX ... ADD COLUMN` all fail on a project with no reservation: `Cannot alter vector index on table <t> because there is no BACKGROUND reservation.` (same message with `search index` substituted). The check fires **before** validation, so these statements cannot even be dry-run — `--dry_run` returns the reservation error rather than `Query successfully validated`, even when the named index does not exist. An autoscale reservation with baseline 0 is the cheap shape; reservation IDs allow lowercase alphanumerics and dashes only, no underscores. When a user asks about rebuilding an index, surface this requirement first — it is the blocker, not the syntax.
- **`last_index_alteration_info` is a `RECORD` and stays `null` until an `ALTER` succeeds.** `bq query --format=csv` refuses to print it (`Cannot print record field ... in CSV format`); use `--format=prettyjson`.
- **A vector index's `PARTITION BY` must match the base table's partitioning expression exactly.** It is not a free choice of index partitioning. Against a table partitioned by `RANGE_BUCKET(id, GENERATE_ARRAY(0, 30000, 5000))`, repeating that expression validates; naming any other column fails with `Invalid Partition By expression`, as does naming a column on an unpartitioned table.
