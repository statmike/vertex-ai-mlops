...
- **The returned `distance` is not a distance.** In hybrid mode it is a reciprocal-rank-fusion score with **k = 60 on the semantic leg and k = 61 on the lexical leg**, both ranks 1-based:

  ```
  distance = 1 - ( 1/(60 + rank_vector) + 1/(61 + rank_lexical) )
  ```

  The legs do not share a rank base, and the asymmetry is determined rather than cosmetic: a row at semantic rank 1 and lexical rank 2 returns `1 - (1/61 + 1/63)` = `0.9677335415040333`, which lands *between* the two shared-base readings of that same rank pair — a shared base of 60 gives `1 - (1/61 + 1/62)` = `0.9674775251189847`, a shared base of 61 gives `1 - (1/62 + 1/63)` = `0.9679979518689196`. Falling between them is the signature of mixed bases: neither single base reproduces the measured value. (`0.9674775251189847` carries two meanings, so keep them apart — as the shared-base-60 reading of ranks 1/2 it is the reading the measurement rules out, and under the true mixed-base law it is separately the best score any row can attain, at rank 1 in *both* legs.) That best attainable score is `1 - (1/61 + 1/62)`, not 0 — and the frequently quoted `1 - 2/61` = `0.96721` is unattainable. Values cluster in a narrow band just under 1 and are **not comparable** to `VECTOR`-mode distances — never reuse a cosine-tuned threshold. Because the score depends only on ranks, `distance_type` has no effect in hybrid mode. *(Measured against BigQuery, not documented by Google — it explains the ordering you see; it is not a contract.)*
