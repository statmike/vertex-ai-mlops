...
| Engine | Apriori-style segment search | Verified bit-for-bit as `ARIMA_PLUS` + `ML.FORECAST` on defaults; the `p_value` reproduces exactly as `2 · (1 − CDF(\|Z\|))` with `Z = absolute_effect ÷ Σ standard_error` — a deliberately conservative variance (the perfect-correlation ceiling) and a quartic CDF approximation — and does not move with `confidence_level` |
