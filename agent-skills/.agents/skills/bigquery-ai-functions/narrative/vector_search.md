...
Nothing observable from outside BigQuery separates them, but **B is the likelier**. `k = 60` over 1-based ranks is canonical reciprocal rank fusion — Cormack, Clarke and Buettcher (2009) — and the default wherever the constant is exposed: Elasticsearch and OpenSearch both name it `rank_constant` and default it to 60, and Spanner and AlloyDB write 60 into their documented SQL. No published implementation uses 61 as the constant. Where 61 *does* appear everywhere is as the rank-1 denominator, `1/(60 + 1)` — precisely the transcription slip that would yield the observed `1/62` on a top-ranked row. The semantic leg here lands on the canonical `1/61` for its own top row, so the extra `+1` sits on the lexical side specifically.
