...
- `bq-ml/workflows/causal_effect` (`AI.CAUSAL_EFFECT`) — the other function under Google's *Augmented analytics* heading, and the one this is most often confused with. Both are TVF shortcuts that hide a `CREATE MODEL`, but they wrap different models and answer different questions: `AI.KEY_DRIVERS` wraps contribution analysis and asks **which segments explain a metric change** (cross-sectional, no time axis required); `AI.CAUSAL_EFFECT` wraps `ARIMA_PLUS` and asks **whether a known intervention at a known time changed the trajectory**. Running this function on a before period against an after period does *not* give you a causal estimate — it tells you where a change concentrated, not whether anything caused it. It lives in the sibling project because it is a forecasting technique wearing an `AI.*` name; the comparison table is in `reference/augmented-analytics.md` (`reference/augmented-analytics.md`).
