...
That is the whole lesson in one row. The function is behaving exactly as documented; the failure is in the design around it. Nor is the result a borderline artifact of a liberal test — the sibling project shows this p-value is computed on the perfect-positive-correlation *ceiling* variance, the most conservative construction available, so `0.044108` is the **largest** p-value that reading of the data supports. A univariate counterfactual can only carry forward patterns it has already seen, and an annual cycle needs more than a year of pre-intervention data before it is one of them. Point the same call at a date just before a seasonal turn, with a pre-window shorter than the cycle, and it will hand you a significant result with no cause behind it.
