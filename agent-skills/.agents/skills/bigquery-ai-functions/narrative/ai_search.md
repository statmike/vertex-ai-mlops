...
Read the two denominators carefully — they are not the same. The semantic leg uses a rank base of 60, the lexical leg 61. A row first semantically and second lexically returns `1 - (1/61 + 1/63)` = `0.9677335415040333`, and neither shared base reproduces that number: 60 on both legs would give `1 - (1/61 + 1/62)` = `0.9674775251189847`, and 61 on both legs `1 - (1/62 + 1/63)` = `0.9679979518689196`. The measured value falls *between* the two, which is the signature of mixed bases. Note that `0.9674775251189847` carries a second meaning that has nothing to do with base symmetry: under the real law it is the score of a row at rank 1 in *both* legs, which is the best score attainable. That is why hybrid scores sit just under `0.97` and a self-match is never `0`.
