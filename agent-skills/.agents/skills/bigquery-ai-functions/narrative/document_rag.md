> **⚠️ NOT CURRENTLY WORKING:** Step 1 of this workflow uses `AI.PARSE_DOCUMENT` (preview), which Google took offline for revision on 2026-06-01. The function does not execute, and its reference documentation is now withdrawn as well: the reference page returns 404, the function is absent from the BigQuery AI functions navigation tree, and it is no longer named in the [generative AI overview](https://cloud.google.com/bigquery/docs/generative-ai-overview). The whole workflow is blocked — Steps 2 through 4 depend on the chunks Step 1 produces. Watch the [BigQuery release notes](https://cloud.google.com/bigquery/docs/release-notes) for status, and see the `functions/ai_parse_document` (`AI.PARSE_DOCUMENT` notebook) for the function's details.
>
> **Interim alternative:** `functions/ml_process_document` (`ML.PROCESS_DOCUMENT`) reaches the same Document AI Layout Parser for OCR, document extraction, and chunking, and it remains documented and supported. Two differences to plan for. It requires a Document AI remote model — a `CREATE MODEL ... REMOTE WITH CONNECTION ... OPTIONS(remote_service_type = 'CLOUD_AI_DOCUMENT_V1', document_processor = 'PROCESSOR_ID')` step — which is precisely the setup `AI.PARSE_DOCUMENT` removed. And it returns a single JSON column, `ml_process_document_result`, instead of the typed `chunk_id` / `start_page` / `end_page` / `content` columns Step 1 below produces, so chunks have to be unpacked from that JSON before the embedding and retrieval steps can use them. Chunking is requested through process options, for example `PROCESS_OPTIONS => (JSON '{"layout_config": {"chunking_config": {"chunk_size": 250, "include_ancestor_headings": true}}}')`.
>
> **The outputs stored below are a record of this workflow running before the withdrawal, not current behavior.** They are kept as evidence of the completed pipeline; the notebook cannot be re-run to reproduce them today.
