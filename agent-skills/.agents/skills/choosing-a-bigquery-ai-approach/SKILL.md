...
10. **Is the task one a pre-trained Google Cloud AI service already solves** — translate, sentiment/entities/syntax, image labels and OCR, speech-to-text, document extraction — **and does the output have to be a record rather than an answer?**
    Yes → the Cloud AI service models: `ML.TRANSLATE`, `ML.UNDERSTAND_TEXT`, `ML.ANNOTATE_IMAGE`, `ML.TRANSCRIBE`, `ML.PROCESS_DOCUMENT`, in the `bigquery-ai-functions` skill under `reference/cloud-ai-service-models.md`. **This is a third branch, not a variant of the other two**: you train nothing, but you also do not prompt anything. A fixed API version, a published per-unit price, a closed vocabulary, and an explicit per-row error when the service can't do it. Route to a generative `AI.*` function instead when the deliverable is *not* the service's output — a summary rather than a transcript, your categories rather than the taxonomy's.
...
- **Pre-trained service vs. generative prompt**: `ML.TRANSLATE`/`ML.UNDERSTAND_TEXT`/`ML.ANNOTATE_IMAGE`/`ML.TRANSCRIBE`/`ML.PROCESS_DOCUMENT` vs. `AI.GENERATE` over the same rows — both sides built and run side by side in each of those five notebooks. The service is stable (nothing moves when a Gemini default does) and tells you when it can't do something; the prompt answers anything, the same confident way, with no status column beside it. **Stable is not the same as deterministic**, though — measured with the cache off, `ML.TRANSCRIBE` disagrees with itself across identical calls while `ML.TRANSLATE` and `ML.UNDERSTAND_TEXT` do not. Frequently the right answer is *both*: the service output as the durable priced column, the generative function over that column.
...
- Generative/prompt-driven tasks, embeddings/search over unstructured content, zero-training tabular prediction/forecasting/anomalies/drivers, document processing, or a pre-trained Cloud AI service (translation, NLP, vision, speech-to-text, Document AI) → [`bigquery-ai-functions` skill](../bigquery-ai-functions/SKILL.md).
