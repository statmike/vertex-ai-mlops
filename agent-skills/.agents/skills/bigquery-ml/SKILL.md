...
5. **Do you already have predictions and just need metrics** — scored elsewhere, or scored here by a model that no longer exists → `ML.METRICS` needs no model at all; see the `ML.METRICS` entries in `reference/regression.md` and `reference/classification.md`

If the ask is ambiguous between BigQuery ML (trained models) and BigQuery's generative AI functions (`AI.GENERATE`, `AI.FORECAST`, `AI.PREDICT`, `AI.CLASSIFY`, etc. — the sibling `bigquery-ai-functions` skill), and you have access to it, consult the `choosing-a-bigquery-ai-approach` skill first — it triages between the two and encodes the specific head-to-head comparisons (e.g. `ARIMA_PLUS` vs. `AI.FORECAST`, and the trained tabular models here vs. `AI.PREDICT`) already worked out in this project. Note that `AI.PREDICT` now does zero-training regression and classification on ordinary feature tables, so a tabular problem no longer routes here automatically. If that skill isn't available, ask the user directly: do they need training-time control (custom holidays/regressors, hierarchical reconciliation, interpretable coefficients, scheduled retraining) or a fast, zero-setup answer?
...
- **Coefficient significance is a train-time decision, not a query-time one**: `ML.ADVANCED_WEIGHTS` needs `calculate_p_values = TRUE` + `DUMMY_ENCODING` + `l1_reg = 0` set at `CREATE MODEL` time and cannot be retrofitted onto a trained model. Worth defaulting to on any `LINEAR_REG`/binary `LOGISTIC_REG` you might want to interpret, because `ML.WEIGHTS` and `ML.GLOBAL_EXPLAIN` cannot distinguish a real coefficient from noise — on a 32k-row `census_adult_income` model, 54 of 97 testable coefficients came back at p > 0.05.
...
- **You do not need the model to compute evaluation metrics.** `ML.EVALUATE` requires the model object and fails with `Not found: Model ...` once it is dropped; `ML.METRICS` (Preview) takes only a relation with an actual column and a predicted column and returns the same regression/classification metrics — so predictions from Vertex AI batch prediction, a vendor scoring API, or an archived forecast can be scored in SQL with no BigQuery ML model involved. Measured on the same predictions, the two agree to at least 13 significant digits.
- **Column *type* silently changes what `precision`/`recall`/`f1_score` mean, in `ML.METRICS` and `AI.EVALUATE` alike.** A `BOOL` actual/predicted pair is scored **binary** — the positive (`TRUE`) class only. The `STRING` rendering of the identical values is scored **multiclass and macro-averaged**, even with just two classes. `accuracy` is the same either way. Measured on one imbalanced problem (12 positives of 62 rows): `precision` 0.5238 as `BOOL` against 0.7497 as `STRING`, from byte-identical predictions. On balanced data the two nearly agree, which is how this hides until it matters. Fix the type deliberately and say which convention a published number uses.
