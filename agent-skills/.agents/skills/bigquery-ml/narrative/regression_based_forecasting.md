...
The two failure modes are opposites, and it's worth knowing both. Plain `LAG` misaligns **loudly**: the lag value is real data, just from the wrong date, and the row count versus the calendar span gives it away (as measured above). The model-free time-series functions in `functions/time_series` (`functions/time_series/`) — `ML.TREND`, `ML.SEASONALITY`, `ML.DETECT_CHANGE_POINTS` — misalign **quietly**: each gap-fills the series to its own calendar span before computing, so the dates always line up, but the values spanning a gap are manufactured rather than observed. Neither is wrong; they fail in ways you check for differently. If you want a calendar-aware view of this series' trend and seasonality without training a model, those functions are the tool — just read anything inside the October 2016 – March 2017 window as interpolation, not observation.
