...
This notebook focuses narrowly on applying Vertex AI Pipelines to BQML. For the much deeper, general-purpose treatment of pipeline mechanics — components, parameters/artifacts, control flow, scheduling, notifications, testing, and managing pipelines/jobs — see [MLOps/Pipelines/](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/readme.md), a full notebook series this one builds on rather than repeats.
...
The prebuilt ops cover `CREATE MODEL` / `ML.EVALUATE` / `ML.PREDICT`, but reading a metric back out and branching on it needs a small custom `@dsl.component` — exactly the pattern of "prebuilt ops first, custom component for the gap they don't cover." For the full range of ways to build a component (prebuilt, lightweight Python, containerized, container) and the full parameter/artifact type system, see [`Vertex AI Pipelines - Components`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Components.ipynb) and [`Vertex AI Pipelines - IO`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20IO.ipynb).
...
- `pipelines/airflow_with_kfp` (`pipelines/airflow_with_kfp/`) — triggers this notebook's compiled pipeline from an Airflow DAG on Managed Airflow, so the BQML logic stays here and Airflow only schedules and waits.
...
**The much deeper, general-purpose Vertex AI Pipelines series** — [MLOps/Pipelines/readme.md](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/readme.md) is the full map; this notebook only needed a slice of it. Most relevant follow-ups for what's used above:
- [`Vertex AI Pipelines - Start Here`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Start%20Here.ipynb) / [`- Introduction`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Introduction.ipynb) — if pipelines are new, start here rather than with this BQML-focused notebook.
- [`Vertex AI Pipelines - Components`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Components.ipynb) — the full range of component types (prebuilt, lightweight Python, containerized, container) beyond the one custom component and three prebuilt ops used here.
- [`Vertex AI Pipelines - IO`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20IO.ipynb) — the full parameter/artifact type system and Vertex ML Metadata lineage, the same mechanism behind the `evaluation_metrics` artifact gotcha above.
- [`Vertex AI Pipelines - Control`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Control.ipynb) — every control-flow construct (ordering, `if`/`elif`/`else`, looping + parallelism, exit handlers, error handling), of which `dsl.If` is only one.
- [`Vertex AI Pipelines - Scheduling`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Scheduling.ipynb) — this notebook submits one-off runs; production use would likely schedule the retrain step instead (compare `pipelines/scheduled_queries/`'s BigQuery-native scheduling of the same idea).
- [`Vertex AI Pipelines - Notifications`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Notifications.ipynb) — alerting on pipeline completion/failure, the KFP-native counterpart to `scheduled_queries`'s `email_preferences.enable_failure_email`.
- [`Vertex AI Pipelines - Managing Pipeline Jobs`](https://github.com/statmike/vertex-ai-mlops/blob/main/MLOps/Pipelines/Vertex%20AI%20Pipelines%20-%20Managing%20Pipeline%20Jobs.ipynb) — listing, filtering, cancelling, and deleting jobs across an environment, beyond this notebook's own polling of two specific runs.
