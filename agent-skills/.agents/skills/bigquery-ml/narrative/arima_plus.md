...
- If all you want is the decomposition — trend, seasonality, or structural change points — and not a forecast or a model object, `functions/time_series` (`functions/time_series/`) does it with `ML.TREND` / `ML.SEASONALITY` / `ML.DETECT_CHANGE_POINTS`, no `CREATE MODEL` at all. Measured on this same Citi Bike series: `ML.SEASONALITY`'s weekly and yearly components are bit-identical to this model's `seasonal_period_*` output, while `ML.TREND` only reaches corr 0.9955 against this model's `trend` and defaults `adjust_step_changes` to FALSE where this model defaults it TRUE.
...
>
> Note what `ML.DETECT_ANOMALIES` is answering here: it flags individual **points** that deviate from the model's forecast. The complementary question — where does this series undergo a *sustained* level shift — is answered by `ML.DETECT_CHANGE_POINTS`, one of the model-free functions covered in `functions/time_series` (`functions/time_series/`). Run against these same five stations, it reports the edges of this very gap as change-point windows, because it gap-fills the same way. Point anomaly and structural change point are different findings on the same data; neither substitutes for the other.
