...
Modernizes `Applied ML/Forecasting/BigQuery ML For Hierarchical Forecasting.ipynb` (retired 2026-07-21 — that notebook was removed from the repo once this rebuild was verified against it feature-for-feature, so this workflow is now the only copy). This workflow assumes the bottom-up mechanics are already understood (see `models/arima_plus/` Step 10 for that verification) and focuses on the harder, uncovered half: building a top-down alternative and comparing its accuracy against the built-in approach at every level of a real hierarchy.

**Data:** [`bigquery-public-data.iowa_liquor_sales.sales`](https://console.cloud.google.com/marketplace/product/bigquery-public-datasets), a real `State → County → City → Store` hierarchy. Scoped to 2 counties (POLK, LINN), 2 cities each (Des Moines/Ankeny; Cedar Rapids/Marion), 2 stores each — 8 stores, 15 series total across all 4 levels. Aggregated to **weekly** totals, not daily — real per-store sale-day coverage is only ~15-30% at daily granularity (most liquor stores don't sell every single day) versus ~90%+ at weekly, so weekly avoids most of the interpolation daily granularity would otherwise require. That aggregation choice is the manual version of what the model-free functions in `functions/time_series` (`functions/time_series/`) do automatically: `ML.TREND`, `ML.SEASONALITY`, and `ML.DETECT_CHANGE_POINTS` gap-fill each series to its own span before computing, so on sparse daily store data they would silently manufacture the 70–85% of days that have no sale at all. Aggregating first is the safer move when coverage is this thin. TEST = the last 8 weeks; `horizon = 8`.
