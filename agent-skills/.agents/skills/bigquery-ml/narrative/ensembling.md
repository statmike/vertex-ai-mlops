...
- `workflows/feature_store` (`workflows/feature_store/`) — the *other* leakage this notebook's 3-way split is guarding against, measured rather than avoided. Stacking leakage is about which rows a model saw; **label leakage** is about which point in time its features came from. That workflow builds one feature history two ways and shows the leaky construction reporting `roc_auc` 0.9387 against the correct 0.5194 — then collapsing to 0.5299 when scored on the features production can actually supply.
- `functions/feature_store` (`functions/feature_store/`) — `ML.FEATURES_AT_TIME` / `ML.ENTITY_FEATURES_AT_TIME`, the retrieval functions that make a point-in-time-correct training set a query rather than a project.
