...
| Data validation/drift functions (`ML.VALIDATE_DATA_SKEW`, `ML.VALIDATE_DATA_DRIFT`, `ML.TFDV_DESCRIBE`, `ML.TFDV_VALIDATE`) | Detects training/serving skew or drift **between two datasets** | You need to monitor a model or pipeline in production for distribution shift. To profile a single dataset instead, see `ML.DESCRIBE_DATA` in [`preprocessing-functions.md`](preprocessing-functions.md#exploratory-data-analysis-profiling-correlation) |
...
**"I need a pre-trained Google Cloud AI service from SQL"** → also `REMOTE WITH CONNECTION`, but a different second option and a different consuming function. `OPTIONS(endpoint = 'https://...')` is *your* deployed endpoint, billed by the hour, consumed with `ML.PREDICT` — that is the row above and everything on this page. `OPTIONS(REMOTE_SERVICE_TYPE = '...')` is a **handle to a Google-run service** — nothing is deployed, nothing is billed hourly, and the consuming function is named after the service: `ML.TRANSLATE`, `ML.UNDERSTAND_TEXT`, `ML.ANNOTATE_IMAGE`, `ML.TRANSCRIBE`, `ML.PROCESS_DOCUMENT`. Those five live in the sibling `bigquery-ai-functions` skill (`reference/cloud-ai-service-models.md`), because the reader never manages a model artifact. Same connection and IAM mechanics; no training, no `ML.EVALUATE`, no endpoint to tear down, and per-row failures in a `*_status` column.
...
- **Not in this family: `ML.DESCRIBE_DATA`.** It answers *what is in this dataset*, from one relation, with no comparison — a different question from the four above. It is documented with `ML.CORRELATION` under exploratory data analysis in [`preprocessing-functions.md`](preprocessing-functions.md#exploratory-data-analysis-profiling-correlation). Profile there, monitor here.
...
- **XGBoost Booster compatibility is a file-extension question in both directions, not a library-version one.** On **import**, what the `XGBOOST` importer accepts is decided by the artifact's extension: the same Booster written by a current `xgboost` (3.x) imports as `model.json` or `model.ubj` and is rejected as `model.bst` (`Invalid XGBoost model: could not load model from file`) — verified with byte-identical `.bst`/`.ubj` uploads, so no version pin is needed as long as you save with a `.json`/`.ubj` extension. On **export**, the file `EXPORT MODEL` writes for `BOOSTED_TREE_*`/`RANDOM_FOREST_*` depends on the training-time `xgboost_version` option: `model.bst` in a legacy binary format at the `0.9` **default** (modern `xgboost` cannot load it — `Check failed: str[0] == '{'`; pin `xgboost==1.7.6`), and `model.ubj` at `'2.1'` (GA 2026-08-27), which current `xgboost` reads unpinned and warning-free. Setting `xgboost_version = '2.1'` therefore also makes the export directly re-importable. Feature names are **not** preserved at either version — reassign `Booster.feature_names` manually.
...
- **Categorical metrics are statistics of the data; the numeric metric is a statistic of a fixed-size histogram.** Reproduced by hand against the function on two disjoint deterministic census populations: `L_INFTY` is the largest absolute difference in category proportion, and `JENSEN_SHANNON_DIVERGENCE` is the base-2 *divergence* (mix the two proportion vectors, average the two KL divergences from the mixture — no square root); both reproduce for `race`/`relationship`/`sex` with a largest disagreement of exactly zero. A **numeric** column is not compared value to value: each input gets its own ten-bucket equal-width histogram over **its own min→max** with fractional counts, the two are realigned onto the union of both edge sets by overlap width, and the base-2 JSD is taken over the aligned pair (those three steps match BigQuery to ~1e-16 on `age`, `education_num`, `hours_per_week`).
- **A numeric drift value is only meaningful alongside the two ranges it was computed from.** Because JSD is maximized by the finest partition, the divergence over a column's *distinct values* is a ceiling for any bucketing of it — yet `education_num` reports **4.5x that ceiling** (0.181256 against 0.039775) purely because one population starts at 1 and the other at 2, offsetting the grids. Appending one row holding `education_num = 1` to the 1,116-row comparison set (a value the base population already holds 51 of) re-cuts the edges and drops the reported drift to 0.036175, `is_anomaly = FALSE`. This is TFDV semantics, not a defect: ten buckets regardless of cardinality, per-dataset edges, and uniform density assumed inside a bucket combine to produce the gap. Read `ML.DESCRIBE_DATA`'s `min`/`max` alongside the drift number; where a numeric range moves for reasons that are not drift, bucketize it yourself (`ML.BUCKETIZE` with fixed split points) and monitor the label as a categorical column so the edges are yours in every window.
...
- [`narrative/model_monitoring.md`](../narrative/model_monitoring.md) (source: `functions/model_monitoring/`) — all four data validation/drift functions on census_adult_income; also used as the conditional-retrain trigger in pipelines/
