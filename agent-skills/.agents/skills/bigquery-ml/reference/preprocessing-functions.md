...
`ML.DESCRIBE_DATA` is one line of SQL with nothing to configure — `SELECT * FROM ML.DESCRIBE_DATA(TABLE project.dataset.table)` returns one row per input column and 20 output columns, populating only the fields that apply to each column's type. Options worth setting explicitly on top of that: `top_k` defaults to **1** (the mode and nothing else, range [1, 10000]) and `num_quantiles` to **2** (min/median/max only, range **[1, 100000]** — the docs give the floor as 2, but 1 is accepted and returns min/max). Both defaults are usually too low to be useful. Pass options positionally as a second `STRUCT` argument; an unrecognized field name is rejected outright (`Found unsupported setting field topk`).
...
- **`ML.DESCRIBE_DATA`'s `quantiles` are approximate and not stable across runs.** They come from `APPROX_QUANTILES`: the identical `STRUCT(3 AS top_k, 4 AS num_quantiles)` call on census `education_num` has returned both `12.0` and `13.0` in the same interior position across repeated cache-off runs. Treat them as a shape sketch, never as a value to compare across queries or to hardcode in prose.
- **Raising `top_k` is how placeholder categories become visible.** At the default `top_k = 1`, census `workclass` shows only its mode; at `top_k => 9` the `' ?'` placeholder appears at rank 4 with 1,836 rows — and 9 equals that column's `unique` count, so the whole distribution fits. Choose the argument form to match: the `TABLE` form profiles the raw table, while the subquery form profiles data you have already repaired (`NULLIF(TRIM(workclass), '?')` plus a `WHERE age >= 25` filter drops census to 26,991 rows and moves `workclass` to 25,787 values / 1,204 nulls / `min` of `Federal-gov`). Change one thing at a time — row filter, null repair, and option values each move different output columns.
...
- [`narrative/exploration.md`](../narrative/exploration.md) (source: `functions/exploration/`) — `ML.DESCRIBE_DATA` + `ML.CORRELATION` on census_adult_income: the bare one-line call and a before/after sweep of `top_k`/`num_quantiles` and the two argument forms, the placeholder-null catch, all three correlation methods reproduced against SciPy and pure SQL, the Kendall cost sweep, and the CUBE row-count match
