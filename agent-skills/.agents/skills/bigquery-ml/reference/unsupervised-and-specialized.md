...
## Model-free time-series functions (no `CREATE MODEL`)

These three are table-valued functions, not model types — they take a relation (a table reference or a parenthesized subquery) as their first argument and return results directly. No model object, no connection, no training. Preview as of 2026-08-20.

| Function | What it returns | Use this when | Key gotcha to know upfront |
|---|---|---|---|
| `ML.TREND` | The de-seasonalized trend component, one row per input row (plus `horizon` forecast rows if asked) | You want the underlying level of a series without fitting a model | `adjust_step_changes` defaults to **FALSE** here but **TRUE** in `ARIMA_PLUS` — the two surfaces disagree out of the box |
| `ML.SEASONALITY` | Five fixed columns (`yearly`, `quarterly`, `monthly`, `weekly`, `daily`), NULL on every row for periods that weren't fitted | You want the seasonal components of a series | The schema is fixed, so an unfitted period is NULL rather than absent — detect it with `COUNTIF(col IS NULL)`, not by reading the schema |
| `ML.DETECT_CHANGE_POINTS` | One row per detected change *window*: `begin_timestamp`, `end_timestamp`, and a `metrics` STRUCT | You want the points where the level of a series shifted and stayed shifted | The function gap-fills before computing, so a data outage manufactures a textbook structural break — profile your gaps first (see gotchas) |

Shared signature: `(TABLE ref_or_subquery, timestamp_col => STRING, data_col => STRING, [id_cols => ARRAY<STRING>], ...)`. Argument names are case-insensitive and order-independent. Unlike `ML.FORECAST`, the timestamp and data columns keep their **original names** in the output; the data column comes back `FLOAT64` regardless of input type. `status` is the empty string `''` on success, never NULL — `WHERE status IS NULL` matches nothing.
...
**"I want to understand a time series" has three answers, not two — start by asking whether the goal is to predict the future or describe the past.** To *describe* — the trend, the seasonal components, or the points where the level shifted — use the model-free TVFs above and skip `CREATE MODEL` entirely. Measured head-to-head on the same series, `ML.SEASONALITY`'s `weekly` and `yearly` output is **bit-identical** to a trained `ARIMA_PLUS`'s `seasonal_period_weekly`/`seasonal_period_yearly` (mean abs diff 0.0, corr 1.0), so if seasonality is all you need, the model buys you nothing. `ML.TREND` is close but not identical (corr 0.9955, mean abs diff 8.337, max 58.555) — the model applies `cleanSpikesAndDips` and `trendSmoothingWindowSize: -1`, neither separable from the other here, so treat the residual as an unexplained option difference rather than a known mechanism.
...
- **`ML.FORECAST`'s prediction interval is not `forecast_value ± Φ⁻¹((1+confidence_level)/2) · standard_error`** — so never back a standard error out of the rendered bounds. The multiplier is constant across horizons but differs from the exact normal quantile, and the difference *changes sign* with the confidence level: `+0.000735` at 0.80, `−0.001782` at 0.90, `−0.003506` at 0.95 (1.956458 used vs. 1.959964), `+0.000889` at 0.98, `+0.011866` at 0.99. Not a Student *t* (always wider than normal), not a constant scale factor (the sign flips) — the pattern of an inverse-normal-CDF approximation. Dividing a 95% width by `2 × 1.959964` gives a value 0.18% too small: invisible on a chart, and enough to reverse the conclusion of any arithmetic check built on it. Use the `standard_error` column.
...
- **All three model-free time-series TVFs gap-fill each series to its own span before computing — on a gapped series this *manufactures* the structural break `ML.DETECT_CHANGE_POINTS` then reports.** This is the single most consequential behavior in that group. On the public Citi Bike table (a dataset-wide 182-day ingestion outage), labelling each detected window by whether it lands on the edge of a >= 3-day gap showed **six of seven windows across five stations were gap edges**, not ridership changes. The practical workflow is two queries: profile the gaps with `LAG(date) OVER (PARTITION BY id ORDER BY date)`, then keep only the change points that don't land on one. Without that filter the alert fires on your ingestion pipeline, not your business.
- **Three time-series surfaces, three different behaviors over the *same* data gap** — verified on the same public Citi Bike series and its 182-day outage. `ARIMA_PLUS` linearly interpolates across the entire span (measured: `194.0` -> a constant `~-0.415`/day decline -> `118.0`, day by day, for all ~183 missing days), so `ML.DETECT_ANOMALIES` returns a real number for every day inside it. `ARIMA_PLUS_XREG` returns `NULL` for the data column, `is_anomaly` and `anomaly_probability` on every day inside it. The model-free TVFs gap-fill each series to its own span, which is what makes `ML.DETECT_CHANGE_POINTS` report the gap's edges as change points. Never assume a value inside a gap means the same thing across these three — establish which surface produced it first.
- **`ML.TREND` defaults `adjust_step_changes` to FALSE while `ARIMA_PLUS` defaults it TRUE.** Verified by differencing rather than by reading docs: the default output minus the explicit-FALSE output summed to exactly 0.0, and minus explicit-TRUE to 91,344.17 on a 1,341-row series (`bq show --format=prettyjson --model` reports `"adjustStepChanges": true` on the model). If you're reconciling a TVF result against a model result, set the option explicitly on both.
- **`horizon` on `ML.TREND`/`ML.SEASONALITY` extends the output rather than windowing it, and the data column is NOT NULL on the appended forecast rows** — it carries the predicted value while `trend` stays near its last fitted level. Only `time_series_type` ('history' vs. 'forecast') distinguishes them, so an unfiltered `AVG(data_col)` silently mixes actuals with predictions.
- **A change window's `metrics` STRUCT describes the gap-filled series, not your raw rows.** On one tested window a raw `GROUP BY` gave n=3, avg 552.33, min 194; the function reported n=7, avg 346.98, min 192.33879781420765 — fractional, despite the underlying metric being an integer `COUNT(*)`.
- **`ML.DETECT_CHANGE_POINTS` and `ML.DETECT_ANOMALIES` answer different questions and can overlap zero** — 79 row-level anomalies (`anomaly_prob_threshold=0.95`) against 2 change windows on the same series had no overlap at all. Qualified: the separation was sharper than typical there because the windows bracketed an interpolated stretch that is smooth by construction.
- **`SEASONALITIES` accepts `YEARLY`/`QUARTERLY`/`MONTHLY`/`WEEKLY`/`DAILY`, case-insensitively**, and the rejection message enumerates the valid set for some invalid inputs (`HOURLY`, `NO_SEASONALITY`, `AUTO`) but not others (`MINUTELY`, `ANNUAL`, `''`) — don't rely on the error text to discover the accepted values.
...
- [`narrative/time_series.md`](../narrative/time_series.md) (source: `functions/time_series/` — `ML.TREND`, `ML.SEASONALITY`, `ML.DETECT_CHANGE_POINTS`)
