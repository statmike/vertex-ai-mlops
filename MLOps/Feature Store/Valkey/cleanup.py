#!/usr/bin/env python3
"""Tear down the billable resources created by the Valkey Feature Store series.
...
Usage (from this directory):
    ...
    uv run python cleanup.py --yes           # delete instance, PSC policy, Pub/Sub
...
data, and is reusable. The expensive resource is the Valkey instance.
...
def load_config():
    if not os.path.exists(CONFIG_PATH):
        sys.exit(
            ...
            'Run the Environment notebook first — its final cell writes this file '
...
def main():
    ap = argparse.ArgumentParser(description='Tear down Valkey Feature Store resources.')
    ...
    print(f'Valkey Feature Store cleanup — project {project}, region {region}')
    print('DRY RUN (no changes). Re-run with --yes to delete.\n' if dry_run else 'DELETING resources...\n')

    print(f'Valkey instance: {cfg["valkey_instance"]}')
    run(['gcloud', 'memorystore', 'instances', 'delete', cfg['valkey_instance'],
         '--location', region, '--project', project, '--quiet'], dry_run)

    print(f'Pub/Sub subscription: {cfg["pubsub_subscription"]}')
    run(['gcloud', 'pubsub', 'subscriptions', 'delete', cfg['pubsub_subscription'],
         '--project', project, '--quiet'], dry_run)

    print(f'Pub/Sub topic: {cfg["pubsub_topic"]}')
    run(['gcloud', 'pubsub', 'topics', 'delete', cfg['pubsub_topic'],
         '--project', project, '--quiet'], dry_run)

    dlt = cfg.get('pubsub_dead_letter_topic')
    if dlt:
        print(f'Pub/Sub dead-letter topic: {dlt}')
        run(['gcloud', 'pubsub', 'topics', 'delete', dlt,
             '--project', project, '--quiet'], dry_run)

    # PSC service connection policy — only delete if no other Valkey instances use it.
    scp = cfg.get('service_connection_policy')
    if scp:
        print(f'PSC service connection policy: {scp} (only if unused by other instances)')
        run(['gcloud', 'network-connectivity', 'service-connection-policies', 'delete', scp,
             '--region', region, '--project', project, '--quiet'], dry_run)
