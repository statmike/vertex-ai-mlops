#!/usr/bin/env python3
"""Tear down the billable resources created by the Spanner Feature Store series.
...
Usage (from this directory):
    ...
    uv run python cleanup.py --yes           # delete the Spanner instance
...
data, and is reusable. The expensive resource is the Spanner instance (deleting
it removes all databases, tables, and change streams within it).
...
def main():
    ap = argparse.ArgumentParser(description='Tear down Spanner Feature Store resources.')
    ...
    print(f'Spanner Feature Store cleanup — project {project}')
    print('DRY RUN (no changes). Re-run with --yes to delete.\n' if dry_run else 'DELETING resources...\n')

    # Deleting the instance removes all databases, tables, and change streams within it.
    print(f'Spanner instance: {cfg["spanner_instance"]} (includes database, tables, change streams)')
    run(['gcloud', 'spanner', 'instances', 'delete', cfg['spanner_instance'],
