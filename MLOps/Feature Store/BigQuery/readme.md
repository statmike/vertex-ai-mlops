...
> **Also in this series: [Bigtable Feature Store](../Bigtable/readme.md), [Spanner Feature Store](../Spanner/readme.md), [Valkey Feature Store](../Valkey/readme.md), and [Vertex AI Feature Store](../vertex/readme.md)**
>
> This BigQuery approach eliminates the online store entirely — **no sync pipelines, no infrastructure to manage, no key design constraints**. For workloads where 20–200ms latency is acceptable, it's the simplest path: features are always fresh, governance is built in, and there's nothing to provision or sync. For sub-10ms key-based lookups with full control, see [Bigtable Feature Store](../Bigtable/readme.md). For **SQL + ACID transactions** at the serving layer, see [Spanner Feature Store](../Spanner/readme.md). For **sub-millisecond in-memory** serving with HNSW vector search, see [Valkey Feature Store](../Valkey/readme.md). For a fully managed solution, see [Vertex AI Feature Store](../vertex/readme.md). All approaches use BigQuery as the offline store. See the [comparison table](../readme.md#choosing-an-approach) in the parent readme.
