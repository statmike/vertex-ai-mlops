...
# Define table schema to preserve order
schema = [
    bigquery.SchemaField("feature_1", "INTEGER"),
    bigquery.SchemaField("feature_2", "FLOAT"),
    bigquery.SchemaField("feature_3", "STRING"),
    bigquery.SchemaField("feature_4", "BOOLEAN"),
    bigquery.SchemaField("entity_key", "STRING"),
]
...
AS SELECT feature_1, feature_2/2 AS feature_2, feature_3, feature_4, entity_key FROM `{internal_table_id}`
"""
...
AS SELECT feature_1, feature_2/2 AS feature_2, feature_3, feature_4, entity_key FROM `{mview_id}`
"""
bq_client.query(create_logical_view_query).result()
print(f"Created logical view {logical_view_id}")

# Get/Create Feature Store
FEATURE_STORE_NAME = PROJECT_ID.replace('-', '_')
try:
    ...
    print(f"Created the feature store:\n{online_store.resource_name}")
